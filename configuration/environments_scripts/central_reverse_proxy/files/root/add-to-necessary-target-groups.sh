...
for tgArn in $(aws elbv2 describe-tags --resource-arns $( echo "$target_groups" | jq -r '.TargetGroups | .[] | .TargetGroupArn') | jq -r '.TagDescriptions | .[] | select(.Tags | any(.Key=="allReverseProxies") ) | .ResourceArn'); do
    ...
    if [[ "$tgArn" != "$nlbArn" ]]; then
        aws elbv2 register-targets --targget-group-arns "$tgArn" --targets Id="${INSTANCE_ID}"
    fi
done
for tgArn in $(aws elbv2 describe-tags --resource-arns $(echo "$target_groups" | jq -r '.TargetGroups | .[] | .TargetGroupArn') | jq -r '.TagDescriptions | .[] | select(.Tags | any(.Key=="CentralReverseProxy") ) | .ResourceArn'); do
