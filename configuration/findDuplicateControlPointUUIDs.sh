...
find "${BASE_FOLDER}" -name '*.txt' | while read FILE; do
  ...
  UUIDs=$( cat "${FILE}" \
   ...
   | sed -e 's/^ControlPoint[0-9]*UUID://' -e 's/ControlPoint[0-9]*DataSheet:.*P1\.UUID=\([^#]*\).*P2\.UUID=\([^#]*\).*$/\1 \2/' )
...
for UUID_FILE in $( find "${TMP_DIR}" -type f ); do
  NUMBER_OF_EVENTS_FOR_UUID=$( wc -l "${UUID_FILE}" )
  if [ ${NUMBER_OF_EVENTS_FOR_UUID} != 1 ]; then
    echo "ControlPoint ID ${UUID} used in multiple events: $( cat "${UUID_FILE}" | tr '\n' ' ' )" >&2
  fi
done
rm -rf "${TMP_DIR}"
