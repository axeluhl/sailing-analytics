...
function route53_change_resource_record(){
	...
	aws_wrapper route53 change-resource-record-sets --hosted-zone-id "$hosted_zone_id" \
	--change-batch "file://${tmpDir}/$change_resource_record_set_file"
}
...
function create_route53_record_file(){
	local json=$(printf '{"Changes":[{"Action":"CREATE","ResourceRecordSet": \
	{"Name": "%s.sapsailing.com","Type": "CNAME","TTL": %s,"ResourceRecords":[{"Value": "%s"}]}}]}' $1 $2 $3)
