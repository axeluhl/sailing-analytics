...
# Get latest release from releases.sapsailing.com
...
function change_admin_password(){
	...
	result=$(change_admin_password_command $1 $2 $3 $4)
...
function get_access_token(){
	...
	access_token=$(get_access_token_command $1 $2 $3)
...
function create_load_balancer_http(){
	...
	json_result=$(create_load_balancer_http_command $1)
...
function create_load_balancer_https(){
	local_echo 'Creating load balancer "$load_balancer_name"...'
	json_result=$(create_load_balancer_https_command $1 $2)
...
function configure_health_check_http(){
	...
	json_result=$(configure_health_check_http_command $1)
...
function configure_health_check_https(){
	local_echo "Configuring load balancer health check..."	
	json_result=$(configure_health_check_https $1)
...
function add_instance_to_elb(){
	...
	json_result=$(add_instance_to_elb_command $1 $2)
...
function route53_change_resource_record(){
	...
	json_result=$(route53_change_resource_record_command $1 $2 $3)
