...
public class ImageController {
	...
	public static Bitmap scaleBitmap(Bitmap bitmap, int newWidth, int newHeight) {
		...
		float left = middleX - (float) bitmap.getWidth() / 2;
		float top = middleY - (float) bitmap.getHeight() / 2;
		canvas.drawBitmap(bitmap, left, top,
				new Paint(Paint.FILTER_BITMAP_FLAG));
		return scaledBitmap;
	}

	public static Bitmap scaleBitmapAspectRatio(Bitmap bitmap, int targetWidth,
			int targetHeight) {
		...
		int newWidth = Math.round(bitmap.getWidth() * aspectRatio);
		if (newWidth < 1) {
			newWidth = 1;
		}
		int newHeight = Math.round(bitmap.getHeight() * aspectRatio);
		if (newHeight < 1) {
			newHeight = 1;
		}
	...
	// http://stackoverflow.com/a/10028267
	...
	public static Bitmap decodeSampledBitmapFromFile(
			ContentResolver contentResolver, Uri uri, String filename,
			int reqWidth, int reqHeight) {
