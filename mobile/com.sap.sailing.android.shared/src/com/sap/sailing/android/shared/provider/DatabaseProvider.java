...
import com.sap.sailing.android.shared.data.database.BaseTable;
...
public class DatabaseProvider extends ContentProvider {
    ...
    static {
        ...
        messageMap.put(MessageTable._TIMESTAMP, MessageTable._TIMESTAMP);
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        if (BuildConfig.DEBUG) {
            String message = "insert: uri=" + uri + " initialValues=[" + initialValues == null ? "null" : initialValues
                    .toString() + "]";
            ExLog.i(getContext(), TAG, message);
        }
        ...
        if (!values.containsKey(BaseTable._TIMESTAMP)) {
            Time time = new Time();
            time.setToNow();
            values.put(BaseTable._TIMESTAMP, time.format2445());
        }
