...
public class WindHelper {
    ...
    public static void isTrackedRace(final Context context, final ManagedRace race) {
        try {
            ...
            NetworkHelper.getInstance(context).executeHttpJsonRequestAsync(request,
    ...
    public static String generateMapURL(Context context, ManagedRace race, boolean showWindCharts,
        ...
        return dataManager.getMapUrl(AppPreferences.on(context).getServerBaseURL(), race, getEventId(context),
                showWindCharts, showStreamlets, showSimulation, showMapControls);
    }
}
