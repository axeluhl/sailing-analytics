...
public class RaceLogPollerTask extends AsyncTask<UtilNew.Pair<Serializable, URL>, PollingResult, Void> {
    ...
    @Override
    protected Void doInBackground(UtilNew.Pair<Serializable, URL>... queries) {
        for (UtilNew.Pair<Serializable, URL> query : queries) {
            if (isCancelled()) {
                return null;
            }
            ...
            try {
                ...
                publishProgress(new PollingResult(true, 
                        new UtilNew.Pair<Serializable, InputStream>(query.getA(), responseStream)));
            } catch (IOException e) {
