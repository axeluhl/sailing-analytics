 ...
 * Activate a set of targets by using {@link ExLog#activate(Target)} or {@link ExLog#activate(EnumSet)}. 
 * Currently available are logging to android.util.Log LogCat and logging to a file on SD-card.
 * 
 * The file logging in very defensive in a way that opening and writing to the file is secured by various
 * exception handlers trying to silence all possible errors.
 *
 */
public class ExLog {
	...
	public synchronized static void i(int eventID, String msg, Context context) {
		...
		i("ID: " + String.valueOf(eventID), System.currentTimeMillis() + ":" + (UNIQUE_ID == null ? "" : UNIQUE_ID) + ":" + (msg == null ? "" : msg));
	}
