...
#### How-to change specs
To change something in the charts and then apply it to the operator you have to do the following steps:
- Change and save the files in `sapsailing-operator/helm-charts/sap-sailing-analytics/templates`
- Build and push the operator `operator-sdk build alstolten/sapsailing-operator:v0.xx.xx` followed by `docker push alstolten/sapsailing-operator:0.xx.xx`
- Change the `sapsailing-operator/deploy/operator.yaml` to use the new tag/version and apply the file to the cluster
...
To connect JMX with Prometheus use the `prometheus/jmx_exporter`[14]. I use Kubernetes configmaps[15] to provide the Sailing Analytics container with the neccessary files. So first create the configmap (the files are provided in /kubernetes folder):
```
k create configmap prometheus-java-agent --from-file=jmx-prometheus-config.yaml --from-file=jmx_prometheus_javaagent-0.13.0.jar
```
After that provide the container spec with a volume:
```
volumes:
 - name: prometheus-java-agent
   configMap:
    name: prometheus-java-agent
```
Mount that volume into the pod:
```
volumeMounts:
 - name: prometheus-java-agent
   mountPath: /home/sailing/servers/server/configuration/prometheus-java-agent
```
Add an additional Java Parameter through an env variable:
```
- name: ADDITIONAL_JAVA_ARGS
  value: -javaagent:/home/sailing/servers/server/configuration/prometheus-java-agent/jmx_prometheus_javaagent-0.13.0.jar=9102:/home/sailing/servers/server/configuration/prometheus-java-agent/jmx-prometheus-config.yaml
```
Add an annotation to the metadata of the pod, to let prometheus know to collect data (make sure you use the same port as specified in the Java Args):
```
annotations:
  prometheus.io/scrape: "true"
  prometheus.io/port: "9102"
```
Do not forget to build and update the operator, if you are using it.
