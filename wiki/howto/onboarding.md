...
This is required only if you intend to run a full solution build locally on your own machine. In all other scenarios, such as launching the solution from within your Eclipse development environment, this part of the set-up is not needed.

Make sure you have Maven 3.9 or above installed. You will also need Java 17 or above. Your ``JAVA_HOME`` environment variable has to point to the Java 17 (or above) installation. Additionally you need to set the ``JAVA8_HOME`` environment variable to point to your Java 8 installation. Should your ``.bash_profile`` or ``.bashrc`` make changes to any of these two environment variables, change those scripts, or comment the lines from ``configuration/buildAndUpdateProduct.sh`` that sources the ``.bash_profile`` script.
