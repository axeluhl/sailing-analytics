...
### LeanDI Build for Android Apps

We currently release the Android apps off a branch called ``android-maven-build-bug3190-release`` and referred to in the sequel as the "Android release branch." This will most likely change to something more handy over time, but the actual branch name doesn't really matter for these explanations as it can easily be substituted at any point in time.

The Android release branch holds the changes required to run a successful Android build in LeanDI. This cannot be overestimated, and credits go to Steffen Schaefer for pulling this off! Check out the specificities for the Android build in the top-level ``pom.xml`` file and you see why this was such a great achievement.
...
Once this is done, a [Customer release build can be started](https://veldirelease.wdf.sap.corp:8443/hudson/job/sapsailingcapture/ldi-customer-release). Enter the ``rel-x.y`` branch name in the "SCM Revision" field, select Maven version 3.1.1 and click the "Build" button. If the build succeeds, Final Assembly should be able to _promote_ the build artifact, such as the ``.apk`` files which should have been signed using the SAP certificate.
