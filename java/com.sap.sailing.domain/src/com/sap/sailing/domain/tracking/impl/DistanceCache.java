...
public class DistanceCache {
    private final NavigableSet<UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>>> distanceCache;
    
    private final NamedReentrantReadWriteLock lock;
    
    private static final Comparator<UtilNew.Pair<TimePoint, ?>> timePointInPairComparator = new Comparator<UtilNew.Pair<TimePoint, ?>>() {
        @Override
        public int compare(UtilNew.Pair<TimePoint, ?> o1, UtilNew.Pair<TimePoint, ?> o2) {
    ...
    public DistanceCache(String nameForLockLogging) {
        ...
        this.distanceCache = new ArrayListNavigableSet<UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>>>(timePointInPairComparator);
    }
     ...
     */
    public UtilNew.Pair<TimePoint, UtilNew.Pair<TimePoint, Distance>> getEarliestFromAndDistanceAtOrAfterFrom(TimePoint from, TimePoint to) {
        LockUtil.lockForRead(lock);
        try {
            UtilNew.Pair<TimePoint, UtilNew.Pair<TimePoint, Distance>> result = null;
            UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> entryForTo = distanceCache.floor(createDummy(to));
            if (entryForTo != null) {
                final UtilNew.Pair<TimePoint, Distance> fromCeiling = entryForTo.getB().ceiling(new UtilNew.Pair<TimePoint, Distance>(from, null));
                if (fromCeiling != null) {
                    result = new UtilNew.Pair<TimePoint, UtilNew.Pair<TimePoint, Distance>>(entryForTo.getA(), fromCeiling);
                }
            }
            return result;
        } finally {
    ...
    public void invalidateAllAtOrLaterThan(TimePoint timePoint) {
        ...
        try {
            UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> dummy = createDummy(timePoint);
            NavigableSet<UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>>> toRemove = distanceCache.tailSet(dummy, /* inclusive */ true);
            for (Iterator<UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>>> i=toRemove.iterator(); i.hasNext(); ) {
                UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> entryToRemove = i.next();
    ...
    private NavigableSet<UtilNew.Pair<TimePoint, Distance>> getEntryForTo(TimePoint to) {
        NavigableSet<UtilNew.Pair<TimePoint, Distance>> result = null;
        UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> dummyForTo = createDummy(to);
        UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> entryForTo = distanceCache.floor(dummyForTo);
    ...
    public void cache(TimePoint from, TimePoint to, Distance distance) {
        LockUtil.lockForWrite(lock);
        try {
            NavigableSet<UtilNew.Pair<TimePoint, Distance>> entryForTo = getEntryForTo(to);
            if (entryForTo == null) {
                entryForTo = new ArrayListNavigableSet<UtilNew.Pair<TimePoint, Distance>>(timePointInPairComparator);
                UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> pairForTo = new UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>>(
                        to, entryForTo);
                distanceCache.add(pairForTo);
            }
            entryForTo.add(new UtilNew.Pair<TimePoint, Distance>(from, distance));
        } finally {
            LockUtil.unlockAfterWrite(lock);
        }
    }
    
    private UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>> createDummy(TimePoint to) {
        return new UtilNew.Pair<TimePoint, NavigableSet<UtilNew.Pair<TimePoint, Distance>>>(to, null);
    }
}
