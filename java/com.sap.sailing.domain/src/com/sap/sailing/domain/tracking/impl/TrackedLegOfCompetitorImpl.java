...
public class TrackedLegOfCompetitorImpl implements TrackedLegOfCompetitor {
     ...
     */
    private SpeedWithBearing getWindwardSpeed(SpeedWithBearing speed, TimePoint at, WindLegTypeAndLegBearingCache cache) throws NoWindException {
    ...
    public Double getGapToLeaderInSeconds(TimePoint timePoint, final Competitor leaderInLegAtTimePoint)
        ...
        return getGapToLeaderInSeconds(timePoint, leaderInLegAtTimePoint, new NoCachingWindLegTypeAndLegBearingCache());
    }
    
    @Override
    public Double getGapToLeaderInSeconds(TimePoint timePoint, final Competitor leaderInLegAtTimePoint, WindLegTypeAndLegBearingCache cache)
            throws NoWindException {
    ...
    public Double getGapToLeaderInSeconds(final TimePoint timePoint) throws NoWindException {
        return getGapToLeaderInSeconds(timePoint, new NoCachingWindLegTypeAndLegBearingCache());
    }

    @Override
    public Double getGapToLeaderInSeconds(final TimePoint timePoint, WindLegTypeAndLegBearingCache cache) throws NoWindException {
        return getGapToLeaderInSeconds(timePoint, new LeaderGetter() {
            @Override
            public Competitor getLeader() {
                return getTrackedLeg().getLeader(hasFinishedLeg(timePoint) ? getFinishTime() : timePoint);
            }
        }, new NoCachingWindLegTypeAndLegBearingCache());
    }
    
    private Double getGapToLeaderInSeconds(TimePoint timePoint, LeaderGetter leaderGetter, WindLegTypeAndLegBearingCache cache) throws NoWindException {
        ...
        // the windward speed
        Speed windwardSpeed = getWindwardSpeed(getTrackedRace().getTrack(getCompetitor()).getEstimatedSpeed(timePoint), timePoint, cache);
        Double result = null;
    ...
    public Speed getVelocityMadeGood(TimePoint at) throws NoWindException {
        if (hasStartedLeg(at)) {
            ...
            return speedOverGround == null ? null : getWindwardSpeed(speedOverGround, timePoint, new NoCachingWindLegTypeAndLegBearingCache());
        } else {
            return null;
        }
    }
