...
public class TrackBasedEstimationWindTrackImpl extends VirtualWindTrackImpl implements RaceChangeListener {
    ...
    public void markPositionChanged(GPSFix fix, Mark mark) {
        ...
        UtilNew.Pair<TimePoint, TimePoint> interval = getTrackedRace().getOrCreateTrack(mark).getEstimatedPositionTimePeriodAffectedBy(fix);
    ...
    protected WindWithConfidence<UtilNew.Pair<Position, TimePoint>> getAveragedWindUnsynchronized(Position p, TimePoint at) {
        ...
        try {
            ...
            WindWithConfidenceImpl<UtilNew.Pair<Position, TimePoint>> result = preResult == null ? null
                    : new WindWithConfidenceImpl<UtilNew.Pair<Position, TimePoint>>(preResult.getObject(), confidenceMultiplier
                            * preResult.getConfidence(),
                    /* relativeTo */new UtilNew.Pair<Position, TimePoint>(p, at), preResult.useSpeed());
            return result;
        } finally {
            unlockAfterRead();
        }
