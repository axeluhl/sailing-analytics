...
public class CandidateChooser implements AbstractCandidateChooser {
    ...
    private void findShortestPath(Competitor co) {
        ...
        LinkedHashMap<Candidate, UtilNew.Pair<Candidate, Double>> candidateWithParent = new LinkedHashMap<>();
        candidateWithParent.put(start, new UtilNew.Pair<Candidate, Double>(null, 0.0));
        UtilNew.Pair<Edge, Double> currentMostLikelyEdge = null;
        while (!candidateWithParent.containsKey(end)) {
            ...
            for (Edge e : all) {
                if (candidateWithParent.containsKey(e.getStart())) {
                    ...
                    if (currentMostLikelyEdge == null) {
                        currentMostLikelyEdge = new UtilNew.Pair<Edge, Double>(e, cost);
            ...
            if (!candidateWithParent.containsKey(currentMostLikelyEdge.getA().getEnd())) {
                candidateWithParent.put(currentMostLikelyEdge.getA().getEnd(), new UtilNew.Pair<Candidate, Double>(currentMostLikelyEdge.getA().getStart(), currentMostLikelyEdge.getB()));
            }
