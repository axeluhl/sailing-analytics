...
/**
 * A caching provider of a competitor and boat map, based on the tracked races and {@link RaceLog}s of the {@link RaceColumn}s of
 ...
 * <ul>
 * <li>a competitor and his boat is registered with or unregistered from a race log of any of the race columns or the regatta log</li>
 ...
 *
 */
public class CompetitorAndBoatProviderFromRaceColumnsAndRegattaLike {
    private static final Logger logger = Logger.getLogger(CompetitorAndBoatProviderFromRaceColumnsAndRegattaLike.class
            .getName());

    private final HasRaceColumnsAndRegattaLike provider;

    private final RegattaLogEventVisitor regattaLogCompetitorsAndBoatsCacheInvalidationListener;

    private final RaceLogEventVisitor raceLogCompetitorsAndBoatsCacheInvalidationListener;

    private final RaceColumnListener raceColumnListener;

    private Map<Competitor, Boat> allCompetitorsAndBoatsCache;

    private final ConcurrentMap<Pair<RaceColumn, Fleet>, Map<Competitor, Boat>> allCompetitorsAndBoatsCacheByRace;

    public CompetitorAndBoatProviderFromRaceColumnsAndRegattaLike(HasRaceColumnsAndRegattaLike provider) {
        super();
        this.provider = provider;
        this.allCompetitorsAndBoatsCacheByRace = new ConcurrentHashMap<>();
        ...
        // method.
        regattaLogCompetitorsAndBoatsCacheInvalidationListener = new BaseRegattaLogEventVisitor() {
            @Override
        ...
        raceLogCompetitorsAndBoatsCacheInvalidationListener = new BaseRaceLogEventVisitor() {
            @Override
    ...
    public Iterable<Competitor> getAllCompetitors() {
        return getAllCompetitorsAndBoats().keySet();
    }

    public Map<Competitor, Boat> getAllCompetitorsAndBoats() {
        if (allCompetitorsAndBoatsCache == null) {
            final Map<Competitor, Boat> result = new HashMap<>();
            boolean hasRaceColumns = false;
            for (RaceColumn rc : provider.getRaceColumns()) {
                hasRaceColumns = true;
                result.putAll(rc.getAllCompetitorsAndTheirBoats());
                for (final Fleet fleet : rc.getFleets()) {
                    rc.getRaceLog(fleet).addListener(raceLogCompetitorsAndBoatsCacheInvalidationListener);
                }
            }
            ...
            if (!hasRaceColumns) {
                ...
                result.putAll(regattaLogProvidedCompetitors);
            }
            // else, don't add regatta log competitors because they have been added in each column already.
            // The competitors and boats are collected from the races. Those, however, will be the regatta log
            // competitors and boats if the race does not define its own.
            ...
            regattaLog.addListener(regattaLogCompetitorsAndBoatsCacheInvalidationListener);
            allCompetitorsAndBoatsCache = result;
        }
        return allCompetitorsAndBoatsCache;
    }

    public Iterable<Competitor> getAllCompetitors(final RaceColumn raceColumn, final Fleet fleet) {
        return getAllCompetitorsAndBoats(raceColumn, fleet).keySet();
    }
    
    /**
     * Obtains competitors and boats from the regatta log and adds those from the tracked race for the raceColumn/fleet
     ...
     */
    public Map<Competitor, Boat> getAllCompetitorsAndBoats(final RaceColumn raceColumn, final Fleet fleet) {
        Pair<RaceColumn, Fleet> key = new Pair<>(raceColumn, fleet);
        Map<Competitor, Boat> result = allCompetitorsAndBoatsCacheByRace.get(key);
        if (result == null) {
            // raceColumn already considers trackedRace, RaceLog and RegattaLog
            final Map<Competitor, Boat> resultMap = new HashMap<>(raceColumn.getAllCompetitorsAndTheirBoats(fleet)); 
            // note: adding listeners is idempotent; at most one occurrence of this listener exists in the race/regatta log's
            // listeners set
            raceColumn.getRaceLog(fleet).addListener(raceLogCompetitorsAndBoatsCacheInvalidationListener);
            provider.addRaceColumnListener(raceColumnListener);
            // consider {@link RegattaLog} competitor changes because the RaceColumns may have added the competitors from there
            provider.getRegattaLike().getRegattaLog().addListener(regattaLogCompetitorsAndBoatsCacheInvalidationListener);
            result = resultMap;
            allCompetitorsAndBoatsCacheByRace.put(key, result);
        }
        return result;
    }

    private void invalidateAllCompetitorsAndBoatsCaches() {
        allCompetitorsAndBoatsCache = null;
        // note: adding listeners was idempotent; at most one occurrence of this listener exists in the regatta log's
        // listeners set
        provider.getRegattaLike().getRegattaLog().removeListener(regattaLogCompetitorsAndBoatsCacheInvalidationListener);
        for (final RaceColumn rc : provider.getRaceColumns()) {
            for (final Fleet fleet : rc.getFleets()) {
                ...
                // log's listeners set
                rc.getRaceLog(fleet).removeListener(raceLogCompetitorsAndBoatsCacheInvalidationListener);
            }
        }
        ...
        allCompetitorsAndBoatsCacheByRace.clear(); // this is a little coarse-grained; but given the typical access patterns it
