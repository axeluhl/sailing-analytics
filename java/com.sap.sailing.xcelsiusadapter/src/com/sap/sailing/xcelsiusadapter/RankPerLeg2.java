...
public class RankPerLeg2 extends Action {
	...
	public void perform() throws Exception {
		...
		for (final TrackedLeg trackedLeg : trackedRace.getTrackedLegs()) {
			...
			final String upOrDownwinLeg = trackedLeg.isUpOrDownwindLeg(time) ? "Upwind"
					: "Downwind";
			...
			for (final Competitor competitor : ranks.keySet()) {
				if (competitorNameSet == null
					...
					// Add empty value in Columng if overallRank/legRank/posGL not higher than 0 
					if (overallRank != 0) {
					...
					addColumn(""
							+ (avgSpeed == null ? "null" : f.format(avgSpeed.getKnots())));
					addColumn(""
							+ (distanceSailed == null ? "null" : f.format(distanceSailed.getMeters())));

					// position and tracking-related columns (see Kersten's mail
