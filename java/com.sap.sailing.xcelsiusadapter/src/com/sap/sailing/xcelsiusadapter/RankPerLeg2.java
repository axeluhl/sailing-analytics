...
public class RankPerLeg2 extends Action {
	...
	public RankPerLeg2(HttpServletRequest req, HttpServletResponse res,
			RacingEventService service, int maxRows) {
	...
	public void perform() throws Exception {
		...
		if (race != null) {
			...
			for (final TrackedLeg trackedLeg : trackedRace.getTrackedLegs()) {
				...
				final String upOrDownwinLeg = trackedLeg
						.isUpOrDownwindLeg(time) ? "U" : "D";
				final String upOrDownwindLegText = trackedLeg
						.isUpOrDownwindLeg(time) ? "Upwind" : "Downwind";
				LinkedHashMap<Competitor, Integer> ranks = trackedLeg
						.getRanks(time);
				// Get competitor data
				for (final Competitor competitor : ranks.keySet()) {
					if (competitorNameSet == null
							|| competitorNameSet.contains(competitor.getName())) {
						// Get data
						final String competitorName = competitor.getName();
						final String nationality = competitor.getTeam()
						...
						TrackedLegOfCompetitor trackedLegOfCompetitor = trackedLeg
								.getTrackedLeg(competitor);
						final int legRank = trackedLegOfCompetitor
								.getRank(time);
						int posGL = 0;
						if (previousLeg != null) {
							posGL = legRank
									- previousLeg.getTrackedLeg(competitor)
											.getRank(time);
						}
						final Double gapToLeader = trackedLegOfCompetitor
								.getGapToLeaderInSeconds(time);
						final double legTime = 1. / 1000. * trackedLegOfCompetitor
								.getTimeInMilliSeconds(time);
						final Speed avgSpeed = trackedLegOfCompetitor
								.getAverageSpeedOverGround(time);
						final Distance distanceSailed = trackedLegOfCompetitor
								.getDistanceTraveled(time);
						...
						addColumn(leg.getTo().getBuoys().iterator().next()
								.getName());
						...
						addColumn(""
								+ (gapToLeader == null ? "null" : gapToLeader));
						addColumn("" + legTime);

						// Number format for round to two places.
						NumberFormat f = new DecimalFormat(".00");
						addColumn(""
								+ (avgSpeed == null ? "null" : f
										.format(avgSpeed.getKnots())));
						addColumn(""
								+ (distanceSailed == null ? "null" : f
										.format(distanceSailed.getMeters())));

						// position and tracking-related columns (see Kersten's
						// mail
						// of 2011-10-07T11:32:00CEST)
						SpeedWithBearing speedOverGround = trackedLegOfCompetitor
								.getSpeedOverGround(time);
						addColumn(""
								+ (speedOverGround == null ? "null"
						...
						addColumn(""
								+ (estimatedTimeToNextMarkInSeconds == null ? "null"
										: estimatedTimeToNextMarkInSeconds));
						Speed velocityMadeGood = trackedLegOfCompetitor
								.getVelocityMadeGood(time);
						addColumn(""
								+ (velocityMadeGood == null ? "null"
