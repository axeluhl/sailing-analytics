...
public class LeaderboardData extends ExportAction {
    ...
    private Element createLeaderboardXML(Leaderboard leaderboard, List<Element> competitors, List<Element> races, UtilNew.Pair<Double, Vector<String>> leaderboardConfidenceAndErrorMessages) {
    ...
    private Element createRaceXML(final TrackedRace race, final Fleet fleet, final List<Element> legs, final RaceColumn column, final Leaderboard leaderboard, int sameDayGroupIndex, int raceCounter, UtilNew.Pair<Double, Vector<String>> raceConfidenceAndErrorMessages) throws NoWindException, IOException, ServletException {
    ...
    private Element createCompetitorXML(Competitor competitor, Leaderboard leaderboard, boolean shortVersion, UtilNew.Pair<Double, Vector<String>> competitorConfidenceAndErrorMessages) throws NoWindException, IOException, ServletException {
    ...
    private Element createLegXML(TrackedLeg trackedLeg, Leaderboard leaderboard, int legCounter, UtilNew.Pair<Double, Vector<String>> raceConfidenceAndErrorMessages, UtilNew.Pair<Double, Vector<String>> legConfidenceAndErrorMessages) throws NoWindException, IOException, ServletException {
        TimePoint timeSpent = MillisecondsTimePoint.now();
        ...
        for (Competitor competitor : trackedLeg.getTrackedRace().getCompetitorsFromBestToWorst(/*timePoint*/ trackedLeg.getTrackedRace().getEndOfRace())) {
            ...
            UtilNew.Pair<GPSFixMoving, Speed> maximumSpeed = competitorLeg.getMaximumSpeedOverGround(legFinishTime);
    ...
    private Element createDataConfidenceXML(UtilNew.Pair<Double, Vector<String>> confidenceAndMessages) {
    ...
    private UtilNew.Pair<Double, Vector<String>> checkData(TrackedLeg leg) throws Exception {
        ...
        return new UtilNew.Pair<Double, Vector<String>>(simpleConfidence, messages);
    }
    
    private UtilNew.Pair<Double, Vector<String>> checkData(TrackedRace race) throws Exception {
        double simpleConfidence = 1.0; Vector<String> messages = new Vector<String>();
    ...
    private UtilNew.Pair<Double, Vector<String>> checkData(Competitor competitor) throws Exception {
        double simpleConfidence = 1.0; Vector<String> messages = new Vector<String>();
    ...
    private UtilNew.Pair<Double, Vector<String>> checkData(Leaderboard leaderboard) throws Exception {
        double simpleConfidence = 1.0; Vector<String> messages = new Vector<String>();
    ...
    public void perform() throws Exception {
        ...
        UtilNew.Pair<Double, Vector<String>> leaderboardConfidenceAndErrorMessages = checkData(leaderboard);
        ...
        for (Competitor competitor : leaderboard.getAllCompetitors()) {
            UtilNew.Pair<Double, Vector<String>> competitorConfidenceAndErrorMessages = checkData(competitor);
        ...
        for (RaceColumn raceColumn : leaderboard.getRaceColumns()) {
            for (Fleet fleet : raceColumn.getFleets()) {
                ...
                if (trackedRace != null && trackedRace.hasGPSData()) {
                    ...
                    UtilNew.Pair<Double, Vector<String>> raceConfidenceAndErrorMessages = checkData(trackedRace);
                    final List<Element> legs = new ArrayList<Element>();
                    int legCounter = 0;
                    for (TrackedLeg leg : trackedRace.getTrackedLegs()) {
                        UtilNew.Pair<Double, Vector<String>> legConfidenceAndErrorMessages = checkData(leg);
