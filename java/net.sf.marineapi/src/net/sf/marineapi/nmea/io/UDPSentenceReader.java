 ...
 * Copyright (C) 2012 Ludovic Drouineau
 *
...
/**
 * As reqular {@link SentenceReader}, detects supported NMEA 0183 sentences from
 * the specified <code>DatagramSocket</code> and dispatches them to registered
 * listeners as sentence events. Each event contains a parser for the read
 * sentence.
 * <p>
...
public class UDPSentenceReader {
     ...
     *
     * @param source DatagramSocket from which to read NMEA data
     */
        ...
        public void run() {
            ...
            while (isRunning) {
                try {
    				...
    				DatagramPacket dataPakt = new DatagramPacket(rcvData, rcvData.length);
    				input.receive(dataPakt);
    				String data = new String(dataPakt.getData(),0,dataPakt.getLength());
