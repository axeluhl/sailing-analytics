...
public class PairingListTemplateImpl implements PairingListTemplate {
	 ...
	 * </p>
	 * <ol>
	 * <li>The <b>general algorithm</b> of creating a pairing list works as
	 * follows: <br \> The decision, who to put into pairing list template, is
	 * taken by looking at our <b>associations</b> (The association matrix
	 * describes how often the competitors encounter each other). The algorithm
	 * of creating a single group follows the following steps:
	 * <ol>
	 * <li>First of all, we generate a random seed, which affects the following
	 * generation. The seed describes a competitor number. By setting the first
	 * competitor in the first group of a flight, we can now go on and select
	 * the rest of competitor to put in the pairing list.</li>
	 * <li>When filling up a single group of a flight, we iterate over all
	 * competitors and check whether the competitor is already set in this
	 * group. Elsewise, we go on finding the smallest sum of encounters with the
	 * remaining competitors in association matrix and the smallest maximum of
	 * encounters. By checking these condition, in the end we will get a well
	 * distributed pairing list template.</li>
	 * </ol>
	 * This is how the algorithm will be applied to the groups: In the first
	 * group the random seed will be set to the first assignment. The other
	 * assignment are generated the way describes above. The remaining groups
	 * unless the last one will be filled up the way the algorithm follows. In
	 * the last group, we just place the remaining competitors that are not
	 * listed in the other groups.</li>
	 * <li>Unfortunately, there is no systematic way of generating a well
	 * distributed pairing list. Since we use a random generated seed, this
	 * algorithm is not deterministic and it works like the trial and error
	 * principle: We commit a count of iterations to the algorithm to create as
	 * much pairing list templates as committed and return the best (see JavaDoc
	 * of getQuality()).<br \> Since we want to improve the performance of this
	 * algorithm, we started to work with <b>concurrency</b>. First we generate
	 * the amount of needed seed combinations (in default case 100.000) and sort
	 * them, so seed combinations with equal seeds for the first x flights are
	 * grouped. After dividing the seeds in that groups we start a task for each
	 * group. The task generates a pairinglist for every seed combination and
	 * returns the best. after that we only have to search the best of all tasks
	 * an return it.</li>
	 * </ol>
	 * </p>
	 * 
	...
	/**
	 * Divides the allSeeds array into parts with seed combinations that starts
	 * with the same seeds. If there are to less flights this method cuts the
	 * off Seeds into 4000 parts to avoid to less Tasks.
	 * 
	 * @param equalSeeds
	 *            number of equal seeds at the beginning of seed combinations
	 *            which should be grouped. Attention: If this number is to big
	 *            there will be very many little parts.
	 * @param allSeeds
	...
	/**
	 * Fast sorting algorithm to sort a huge amount of seeds in max
	 * <code>n*log(n)</code> time. Uses counting sort.
	 * 
	 * @param allSeeds
	 *            array of seeds, that should be sorted
	 * @param competitorCount
	 *            highest value inside the complete array. Needed for counting
	 *            sort.
	...
	class CreateFlightsForListOfSeeds implements Callable<int[][]> {
		...
		/**
		 * This task creates complete pairinglists for a given amount of seed
		 * combinations and returns its best result. The aim is, to give a task
		 * seed combinations with some equals at the beginning, so some flights
		 * does not need to be calculated every time. Its recommended to sort
		 * the seeds and divide it into senseful parts before starting this
		 * task.
		 * 
		 * @param flights
		 *            count of flights
		 ...
		 *            count of competitors
		 * @param seeds
		 *            array with all seed combinations which should be
		 *            calculated in this task
		 * @param equals
		 *            number of equal seeds at the beginning of all seed
		 *            combinations
		 */
	 ...
	 *            current matrix that describes how often a competitor competed
	 *            against another competitor.
	 * @param seed
	 *            generation bases on this competitor number.
	 * 
	 * @return generated flight as
	 *         <code>int[groups][competitors / groups]</code> array
	 */
	...
	/**
	 * This method is called in every task to generate a number of different
	 * pairing lists to compare and choose the best.
	 * 
	 * @param flightCount
	 *            count of flights
	 * @param groupCount
	...
	/**
	 * Returns a matrix that describes how often the competitors competed on a
	 * boat. The matrix has the following dimension:
	 * <code>assignmentAssociations[competitors][boats]</code>.
	 * 
	 * @param pairingList
	 *            current pairing list from which the associations will be
	 *            created
	 * @param associations
	 ...
	 * Method does not change the order of groups or flights. The standard
                    ...
                    for (int zGroup = 0; zGroup < pairingList.length; zGroup++) {
                            ...
                            for (int zPlace = 0; zPlace < competitors * 50; zPlace++) {
                                    ...
                                    if (groupAssignments[position[0]][position[1]] > averageAssignments - 1
                                            ...
                                            int temp = 0;
                                            int bestPosition = this.getBestPositionToChangeTo(groupAssignments[position[0]], prevPosition, this.boatChangeFactor);
	...
	/**
	 * Improves the Assignment changes between two flights by switching the
	 * groups inside a flight. Competitors and there assignment will not be
	 * changed.
	 * 
	 * @param pairingList
	 *            complete pairingListTemplate which needs to be improved
	 * @param flights
	 *            number of flights which was used to generate the
	 *            pairingListTemplate
	 * @param competitors
	 *            number of competitors which was used to generate the
	 *            pairingListTemplate
	 * 
	 ...
	 * <br />
	 * 
	 * Flight 1, Flight2, Flight3 ... -> Flight 1, FLight 1, Flight 1..., Flight
	 * 2, Flight 2, Flight 2...
	 * 
	 * @param bestPLT
	 ...
	 *            reference value
	 * @return int array with 2 indices that represent row and column of the
	 *         worst value
	 */
	...
	/**
	 * Creates a matrix that describes how often the team competes against each
	 * other. Its dimensions are
	...
	/**
	 * Opposite to <code>incremetAssociations</code>. This method reduces the
	 * matrix <code>associtaions</code> by the given pairinglist/flight.
	 * 
	 * @param pairingList
	 *            pairinglist/part of pairinglist/flight which should be removed
	 *            from associations
	 * @param associations
	 *            current associations which should be modified inside this
	 *            method
	 ...
	 * The method ignores all numbers smaller 0, because senseless values are
	 * marked with -1. The return value is used to compare the quality of
	 * different pairing list templates.
	 * 
	 * @param associations
	 *            association describes a 2 dimensional array of integers, which
	 *            contains the information about how often the teams play
	 *            against each other
	 * @return standardDev returns how much the association values deviate from
	 *         each other
	 */
	...
	/**
	 * This Method replaces the given number of dummy competitors with -1, so
	 * they are not longer valid and could not be read on client side.
	 * 
	 * @param bestPLT
