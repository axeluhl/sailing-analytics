...
public class ManeuverCurveBoundariesWithDetailedManeuverLossJsonSerializer
    ...
    public JSONObject serialize(ManeuverCurveBoundaries curveBoundaries) {
        ...
        if(curveBoundaries instanceof HasDetailedManeuverLoss) {
            ...
            result.put(DISTANCE_SAILED_WITHIN_MANEUVER_IN_METERS, details.getDistanceSailedWithinManeuver().getMeters());
            result.put(DISTANCE_SAILED_WITHIN_MANEUVER_TOWARD_MIDDLE_ANGLE_PROJECTION_IN_METERS, details.getDistanceSailedWithinManeuverTowardMiddleAngleProjection().getMeters());
            result.put(DISTANCE_SAILED_IF_NOT_MANEUVERING_IN_METERS, details.getDistanceSailedIfNotManeuvering().getMeters());
            result.put(DISTANCE_SAILED_IF_NOT_MANEUVERING_TOWARD_MIDDLE_ANGLE_PROJECTION_IN_METERS, details.getDistanceSailedTowardMiddleAngleProjectionIfNotManeuvering().getMeters());
        }
        return result;
    }
