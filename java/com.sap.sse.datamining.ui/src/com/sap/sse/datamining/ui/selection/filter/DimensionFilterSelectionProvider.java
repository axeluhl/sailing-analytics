...
import com.sap.sse.datamining.ui.execution.ManagedDataMiningQueryCallback;
import com.sap.sse.datamining.ui.execution.SimpleManagedDataMiningQueriesCounter;
...
class DimensionFilterSelectionProvider {
    ...
    public DimensionFilterSelectionProvider(DataMiningServiceAsync dataMiningService, ErrorReporter errorReporter,
            DataMiningSession session, RetrieverLevelFilterSelectionProvider retrieverLevelSelectionProvider) {
        ...
        selectionTable.addSelectionChangeHandler(new FilterSelectionChangedListener() {
            ...
            public void selectionChanged() {
                DimensionFilterSelectionProvider.this.retrieverLevelSelectionProvider
                        .dimensionFilterSelectionChanged(DimensionFilterSelectionProvider.this);
            }
        });
    ...
    void fetchAndUpdateAvailableData(
     ...
     * 
     * @param isUpdate
     *            <code>true</code>, if the call should update the data and preserve the selection and
     *            <code>false</code>, if it should override the data and clear the selection.
     * @param selectionChangedCallback
     *            An {@link AsyncCallback} to react, if the selection has been changed or not.
     */
    private void fetchAndDisplayAvailableData(final boolean isUpdate, final boolean notifyListenersWhenSelectionChanged,
        ...
        HashMap<DataRetrieverLevelDTO, SerializableSettings> retrieverSettingsDTO = retrieverLevelSelectionProvider
                .getRetrieverSettings();
        HashMap<DataRetrieverLevelDTO, HashMap<FunctionDTO, HashSet<? extends Serializable>>> filterSelectionDTO = retrieverLevelSelectionProvider
                .getCompleteFilterSelection();
        ...
        dataMiningService.getDimensionValuesFor(session, retrieverLevelSelectionProvider.getDataRetrieverChain(),
                ...
                new ManagedDataMiningQueryCallback<HashSet<Object>>(counter) {
                    @Override
                    protected void handleSuccess(QueryResultDTO<HashSet<Object>> result) {
                        ...
                        if (!isUpdate || content.isEmpty()) {
                            ...
                            selectionChanged = selectionTable.updateContent(content,
                                    notifyListenersWhenSelectionChanged);
                        }
                        ...
                        if (retrieverLevelSelectionProviderIterator != null) {
                            if (selectionChanged) {
                                ...
                                retrieverLevelSelectionProvider.updateAvailableData(getSelectedDimension(),
                                        retrieverLevelSelectionProviderIterator);
                            }
                        }
                    }
                    ...
                    protected void handleFailure(Throwable caught) {
                        errorReporter.reportError(
                                "Error fetching the dimension values of " + dimension + ": " + caught.getMessage());
                    }
                });
    }
