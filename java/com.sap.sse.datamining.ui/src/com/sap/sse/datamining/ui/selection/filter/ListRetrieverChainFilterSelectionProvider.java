...
public class ListRetrieverChainFilterSelectionProvider extends AbstractDataMiningComponent<SerializableSettings>
    ...
    /**
     * When the {@link #retrieverLevelList retriever levels} have been received, this field is initialized with the
     * mapping of dimension functions to the reduced set shown in the per-level filters. This way, an original dimension
     * can be {@link ReducedDimensionsDTO#getReducedDimension(FunctionDTO) mapped} to its corresponding dimension from
     * the reduced set of dimensions that are shown in the per-level filters.
     */
    ...
    public void setHighestRetrieverLevelWithFilterDimension(FunctionDTO dimension, Serializable groupKey) {
        FunctionDTO dimensionMappedToReducedDimensions = reducedDimensions == null ? dimension
                : reducedDimensions.getReducedDimension(dimension);
        for (final DataRetrieverLevelDTO retrieverLevel : retrieverChain.getRetrieverLevels()) {
            final RetrieverLevelFilterSelectionProvider selectionProvider = selectionProvidersMappedByRetrievedDataType
                    .get(retrieverLevel);
    ...
    private void updateRetrievalLevels() {
        ...
        dataMiningService.getReducedDimensionsMappedByLevelFor(retrieverChain,
                LocaleInfo.getCurrentLocale().getLocaleName(), new AsyncCallback<ReducedDimensionsDTO>() {
                    @Override
                    public void onFailure(Throwable caught) {
                        errorReporter
                                .reportError("Error fetching the dimensions of the retrieval chain from the server: "
                                        + caught.getMessage());
                    }
                    ...
                    public void onSuccess(ReducedDimensionsDTO dimensionsMappedByLevel) {
                        ...
                        for (Entry<DataRetrieverLevelDTO, HashSet<FunctionDTO>> dimensionsEntry : dimensionsMappedByLevel
                            ...
                            if (!dimensionsEntry.getValue().isEmpty()) {
                                ...
                                RetrieverLevelFilterSelectionProvider selectionProvider = new RetrieverLevelFilterSelectionProvider(
                                        ListRetrieverChainFilterSelectionProvider.this, getComponentContext(), session,
                                        dataMiningService, errorReporter,
                                        ListRetrieverChainFilterSelectionProvider.this, retrieverChain, retrieverLevel);
                                ...
                                firstFilterableRetrieverLevel = retrieverLevel
                                        .getLevel() < firstFilterableRetrieverLevel ? retrieverLevel.getLevel()
                                                : firstFilterableRetrieverLevel;
                            }
                        }
                        retrieverLevelSelectionModel
                                .setSelected(retrieverChain.getRetrieverLevel(firstFilterableRetrieverLevel), true);
                    }
                });
    }
    ...
    void retrieverLevelFilterSelectionChanged(
            RetrieverLevelFilterSelectionProvider retrieverLevelFilterSelectionProvider,
            DimensionFilterSelectionProvider dimensionFilterSelectionProvider) {
        if (!blockDataUpdates) {
    ...
    void updateFilterSelectionProviders(DataRetrieverLevelDTO beginningWithLevel,
            final FunctionDTO exceptForDimension) {
        ...
        if (retrieverLevelToUpdate != null) {
            selectionProvidersMappedByRetrievedDataType.get(retrieverLevelToUpdate)
    ...
    public HashMap<DataRetrieverLevelDTO, HashMap<FunctionDTO, HashSet<? extends Serializable>>> getSelection() {
        ...
        for (RetrieverLevelFilterSelectionProvider selectionProvider : selectionProvidersMappedByRetrievedDataType
                .values()) {
            Map<FunctionDTO, HashSet<? extends Serializable>> levelFilterSelection = selectionProvider
                    .getFilterSelection();
    ...
    public void applyQueryDefinition(StatisticQueryDefinitionDTO queryDefinition) {
        for (RetrieverLevelFilterSelectionProvider selectionProvider : selectionProvidersMappedByRetrievedDataType
                .values()) {
            Map<DataRetrieverLevelDTO, HashMap<FunctionDTO, HashSet<? extends Serializable>>> filterSelection = queryDefinition
                    .getFilterSelection();
        ...
        public RetrieverLevelSelectionModel() {
            ...
            addSelectionChangeHandler(new SelectionChangeEvent.Handler() {
                ...
                public void onSelectionChange(SelectionChangeEvent event) {
                    selectionPanel.setWidget(selectionProvidersMappedByRetrievedDataType
                            .get(retrieverLevelSelectionModel.getSelectedObject()).getEntryWidget());
                }
            });
        }
