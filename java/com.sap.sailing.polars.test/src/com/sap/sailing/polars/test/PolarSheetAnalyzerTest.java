...
public class PolarSheetAnalyzerTest {
    ...
    private PolarDataService createMockedPolarDataService() throws NotEnoughDataHasBeenAddedException {
        ...
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                ...
                new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(8.478048671702888), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(50)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(8.466303997538812), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(51)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(8.583383077026435), 0.5, 1));
        
        //Starboard Downwind
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(141)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(13.794354053931528), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(142)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(13.665782579628802), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(143)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(14.169301888320263), 0.5, 1));
        
        //Port Upwind
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(-51)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(8.445599410456111), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(-50)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(8.553274292235153), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(-49)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(8.614582090896583), 0.5, 1));
        
        //Port Downwind
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(-145)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(13.78894715705271), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(-144)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(13.420656294986587), 0.5, 1));
        when(mockedPolarDataService.getSpeed(any(BoatClass.class), argThat(new SpeedMatcher(14)),
                argThat(new BearingMatcher(-143)))).thenReturn(
        new SpeedWithConfidenceImpl<Integer>(new KnotSpeedImpl(13.268607457651942), 0.5, 1));
    ...
    private class BearingMatcher extends ArgumentMatcher<Bearing> {
        ...
        public boolean matches(Object argument) {
            ...
            if (argument != null) {
                ...
                if (bearing.getDegrees() > bearingToMatchInDegrees - 0.05
                        && bearing.getDegrees() < bearingToMatchInDegrees + 0.05) {
                    result = true;
                }
            }
            return result;
