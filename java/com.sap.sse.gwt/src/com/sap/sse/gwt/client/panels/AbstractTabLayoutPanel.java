 ...
 */
public abstract class AbstractTabLayoutPanel extends ResizeComposite implements HasWidgets, ProvidesResize,
        IndexedPanel.ForIsWidget, AnimatedLayout, HasBeforeSelectionHandlers<Integer>, HasSelectionHandlers<Integer> {
    ...
    private class Tab extends SimplePanel {
        ...
        public boolean remove(Widget w) {
            /*
             * Removal of items from the TabBar is delegated to the TabLayoutPanel to ensure consistency.
             */
            int index = tabs.indexOf(this);
            if (replacingWidget || index < 0) {
                /*
                 * The tab contents are being replaced, or this tab is no longer in the panel, so just remove the
                 * widget.
                 */
    ...
    /**
     * This extension of DeckLayoutPanel overrides the public mutator methods to prevent external callers from adding to
     * the state of the DeckPanel.
     * <p>
     * Removal of Widgets is supported so that WidgetCollection.WidgetIterator operates as expected.
     * </p>
     * <p>
     * We ensure that the DeckLayoutPanel cannot become of of sync with its associated TabBar by delegating all
     * mutations to the TabBar to this implementation of DeckLayoutPanel.
     * </p>
     */
    private class TabbedDeckLayoutPanel extends DeckLayoutPanel {
        ...
        public boolean remove(Widget w) {
            /*
             * Removal of items from the DeckLayoutPanel is delegated to the TabLayoutPanel to ensure consistency.
             */
     ...
     */
    public AbstractTabLayoutPanel(double barHeight, Unit barUnit) {
        ...
        deckPanel.addStyleName(getContentConteinerStyle());
        panel.add(deckPanel);
    ...
    public boolean remove(int index, boolean fireEvents) {
        ...
        child.removeStyleName(getContentStyle());
     ...
     *
     * Use care when setting an object's HTML; it is an easy way to expose script-based security problems. Consider
     * using {@link #setTabHTML(int, SafeHtml)} or {@link #setTabText(int, String)} whenever possible.
     *
     * @param index
