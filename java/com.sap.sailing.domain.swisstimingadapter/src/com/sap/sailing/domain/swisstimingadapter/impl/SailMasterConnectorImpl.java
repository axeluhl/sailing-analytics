...
public class SailMasterConnectorImpl extends SailMasterTransceiverImpl implements SailMasterConnector, Runnable {
    ...
    private void notifyListenersTMD(SailMasterMessage message) {
        ...
        List<UtilNew.Triple<Integer, Integer, Long>> markIndicesRanksAndTimesSinceStartInMilliseconds = new ArrayList<UtilNew.Triple<Integer,Integer,Long>>();
        for (int i = 0; i < count; i++) {
            ...
            markIndicesRanksAndTimesSinceStartInMilliseconds.add(new UtilNew.Triple<Integer, Integer, Long>(markIndex, rank, timeSinceStartInMilliseconds));
        }
        for (SailMasterListener listener : getListeners()) {
            try {
    ...
    private void notifyListenersCAM(SailMasterMessage message) throws ParseException {
        List<UtilNew.Triple<Integer, TimePoint, String>> clockAtMarkResults = parseClockAtMarkMessage(message);
        for (SailMasterListener listener : getListeners()) {
            try {
    ...
    @Override
    public Map<Integer, UtilNew.Pair<Integer, Long>> getMarkPassingTimesInMillisecondsSinceRaceStart(String raceID, String boatID)
        ...
        Map<Integer, UtilNew.Pair<Integer, Long>> result = new HashMap<Integer, UtilNew.Pair<Integer, Long>>();
        for (int i=0; i<count; i++) {
            ...
            result.put(Integer.valueOf(markTimeDetail[0]), new UtilNew.Pair<Integer, Long>(
    ...
    @Override
    public List<UtilNew.Triple<Integer, TimePoint, String>> getClockAtMark(String raceID) throws ParseException, UnknownHostException, IOException, InterruptedException {
        SailMasterMessage response = sendRequestAndGetResponse(MessageType.CAM, raceID);
        String[] sections = response.getSections();
        assertResponseType(MessageType.CAM, response);
        assertRaceID(raceID, sections[1]);
        List<UtilNew.Triple<Integer, TimePoint, String>> result = parseClockAtMarkMessage(response);
        return result;
    }

    private List<UtilNew.Triple<Integer, TimePoint, String>> parseClockAtMarkMessage(SailMasterMessage clockAtMarkMessage) throws ParseException {
        assertMessageType(MessageType.CAM, clockAtMarkMessage);
        List<UtilNew.Triple<Integer, TimePoint, String>> result = new ArrayList<UtilNew.Triple<Integer,TimePoint,String>>();
        ...
        for (int i=0; i<count; i++) {
            ...
            result.add(new UtilNew.Triple<Integer, TimePoint, String>(
