...
localRepo="file:/`readlink -f ../../com.sap.sailing.targetplatform.base/target/repository/ | sed 's/^\/cygdrive\/\([a-zA-Z]\)\//\1:\//'`"
fi
if [ "$OSTYPE" == "msys" ]; then
localRepo="file:/`readlink -f ../../com.sap.sailing.targetplatform.base/target/repository/ | sed 's/^\/\([a-zA-Z]\)\//\1:/\/'`"
fi
