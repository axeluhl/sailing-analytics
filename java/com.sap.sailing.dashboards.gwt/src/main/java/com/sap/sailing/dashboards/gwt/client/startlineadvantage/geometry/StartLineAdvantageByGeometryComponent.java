...
public class StartLineAdvantageByGeometryComponent extends LiveAverageComponent implements PollsLiveDataEvery5Seconds {
    ...
    public StartLineAdvantageByGeometryComponent(DashboardClientFactory dashboardClientFactory) {
        ...
        StartLineAdvantageByGeometryRessources.INSTANCE.gss().ensureInjected();
        this.header.setInnerText(stringConstants.dashboardStartlineAdvantageByGeometry());
        this.header.addClassName(StartLineAdvantageByGeometryRessources.INSTANCE.gss().startLineAdvantageComponent_header());
        liveAveragePanel.getElement().addClassName(StartLineAdvantageByGeometryRessources.INSTANCE.gss().startLineAdvantageComponent_liveAveragePanel());
        livePanel.getElement().addClassName(StartLineAdvantageByGeometryRessources.INSTANCE.gss().startLineAdvantageComponent_livePanel());
        middleLine.getElement().addClassName(StartLineAdvantageByGeometryRessources.INSTANCE.gss().startLineAdvantageComponent_middleLine());
        averagePanel.getElement().addClassName(StartLineAdvantageByGeometryRessources.INSTANCE.gss().startLineAdvantageComponent_averagePanel());
