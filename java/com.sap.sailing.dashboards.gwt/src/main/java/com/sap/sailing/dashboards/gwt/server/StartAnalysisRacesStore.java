...
/**
 * The task of StartAnalyisisRacesStore is to cache StartAnalysisDTOs for
 * Competitors at one day. At initialization it generates StartAnalysisDTOs for
 * Competitors from races started before initialization time and generates
 * StartAnalysisDTOs for Competitors from races that are starting when the
 * server is running. It achieves this basically by registering as
 * RaceChangeListener and receiving so markRoundings for competitors.
 */
public class StartAnalysisRacesStore {
	...
	/**
	 * {@link StartAnalyisisRacesStoreListener} get notified at after
	 * initialization and after the change of
	 * {@link #startAnalysisDTOListForCompetitorName}
	 * */
	private List<StartAnalysisRacesStoreListener> listeners;

	/**
	 * Caches a list of chronologically sorted {@link StartAnalysisDTO} for
	 * names from Competitors. After any change of this field
	 * {@link #notifyListernersAboutStartAnalysisDTOsChanges()} get called.
	 * */
	private Map<String, List<StartAnalysisDTO>> startAnalysisDTOListForCompetitorName;

	/**
	 * Contains {@link StartAnalysisCompetitorDTO}s for every competitor name in
	 * every race. This map gets created for a tracked race when the first mark
	 * one rounding of a tracked race gets received. Caching
	 * {@link StartAnalysisCompetitorDTO}s brings the advantage of faster code
	 * at creating finished {@link #startAnalysisDTOListForCompetitorName}
	 * */
//	private Map<TrackedRace, Map<String, StartAnalysisCompetitorDTO>> startAnalysisCompetitorDTOsForCompetitorNameForTrackedRace;
	...
	/**
	 * Like {@link startAnalysisCompetitorDTOsForCompetitorNameForTrackedRace}
	 * gpsFixesForCompetitorNameForTracedRace contains the list of gps fixes for
	 * a competitor name at from 6 seconds at the start. Caching
	 * {@link gpsFixesForCompetitorNameForTracedRace}s brings the advantage of
	 * faster code at creating finished
	 * {@link #startAnalysisDTOListForCompetitorName}.
	 * */
	...
	/**
	 * Calls
	 * {@link #generateStartAnalysisDTOsForCompetitorNamesFromRacesStartedBeforeClassInitialization()}
	 * to create startanalysisDTO for races that started before class
	 * initialization.
	 */
...
//		startAnalysisDTOWithStaticData = new HashMap<TrackedRace, StartAnalysisDTO>();
	 ...
	 * {@link #markPassingReceived(Competitor, Map, Iterable, TrackedRace)} to
	 * identify just markpassings at mark one, because the position of a boat at
	 * mark one defines the performance/startingrank of the race
	 */
	private int getIndexOfMarkPassingMark(Iterable<MarkPassing> markPassings,
			TrackedRace trackedRace) {
		List<MarkPassing> markPassingsList = listFromIterator(markPassings
				.iterator());
		return trackedRace
				.getRace()
				.getCourse()
				.getIndexOfWaypoint(
						markPassingsList.get(markPassingsList.size() - 1)
								.getWaypoint());
	}
	
	private List<Competitor> getOrCreateAndAddRankingListAtMarkOneForTrackedRace(
			TrackedRace trackedRace) {
		if (!rankingListAtMarkOneForTrackedRace.containsKey(trackedRace)) {
			...
			rankingListAtMarkOneForTrackedRace.put(trackedRace,
					rankingListAtMarkOneForRace);
		}
		return rankingListAtMarkOneForTrackedRace.get(trackedRace);
	}
	
	private List<StartAnalysisDTO> getOrCreateAndAddStartAnalysisDTOListForCompetitorName(
	...
	private StartAnalysisDTO createStartAnalysisDTOForCompetitor(Competitor competitor,
			TrackedRace trackedRace) {
		...
		for (int i = 0; i < MINIMUM_NUMBER_OF_COMPETITORS_IN_STARTANALYSISRANKINGTABLE; i++) {
			competitors.add(createStartAnalysisCompetitorDTO(trackedRace, i+1, getOrCreateAndAddRankingListAtMarkOneForTrackedRace(trackedRace).get(i)));
			if (rankingList.get(i).equals(competitor)) {
				isCompetitorBetweenFirstThree = true;
			}
		}
		if (!isCompetitorBetweenFirstThree) {
			competitors.add(createStartAnalysisCompetitorDTO(trackedRace, getOrCreateAndAddRankingListAtMarkOneForTrackedRace(trackedRace).size(), competitor));
		}
	...
	private StartAnalysisDTO createStartAnalysisDTOWithOnlyStaticData(
			TrackedRace trackedRace) {
	...
	private Map<String, Date> createPerCompetitorIdAsString(
			TrackedRace currentRace, Date fiveSecondsBeforeStart) {
		Map<String, Date> fromPerCompetitorIdAsString = new HashMap<String, Date>();
		try {
			for (Competitor competitor : currentRace
					.getCompetitorsFromBestToWorst(new MillisecondsTimePoint(
							new Date().getTime()))) {
				fromPerCompetitorIdAsString.put(competitor.getId().toString(),
						fiveSecondsBeforeStart);
			}
	...
	private StartAnalysisRankingTableEntryDTO createRankTableEntry(
			TrackedRace trackedRace, int rank, Competitor competitor) {
	...
	private StartAnalysisRankingTableEntryDTO createStartAnalysisRankTableEntryDTOWithRankAndStartTimepoint(
		...
		tableentry.speedAtStartTime = trackedRace.getSpeed(competitor, 1)
				.getKnots();
		tableentry.distanceToLineAtStartTime = trackedRace
				.getDistanceToStartLine(competitor, 1).getMeters();
	...
	private List<PositionDTO> getStartLineBoyPositions(TrackedRace trackedRace) {
		return getMarkPositionDTOs(trackedRace.getStartTimeReceived(),
				trackedRace, trackedRace.getRace().getCourse()
						.getFirstWaypoint());
	}

	private List<PositionDTO> getMarkPositionDTOs(TimePoint timePoint,
			TrackedRace trackedRace, Waypoint waypoint) {

		List<PositionDTO> markPositionDTOs = new ArrayList<PositionDTO>();
		for (Mark startMark : waypoint.getMarks()) {
			final Position estimatedMarkPosition = trackedRace
					.getOrCreateTrack(startMark).getEstimatedPosition(
							timePoint, /* extrapolate */false);
			if (estimatedMarkPosition != null) {
				markPositionDTOs.add(new PositionDTO(estimatedMarkPosition
						.getLatDeg(), estimatedMarkPosition.getLngDeg()));
			}
		}
	...
	private List<MarkDTO> getStartLineMarks(TrackedRace trackedRace) {
		Waypoint startWayPoint = trackedRace.getStartLine(
				trackedRace.getStartTimeReceived()).getWaypoint();
		...
		for (Mark startMark : startWayPoint.getMarks()) {
			...
			if (estimatedMarkPosition != null) {
				startLineMarks.add(convertToMarkDTO(startMark,
						estimatedMarkPosition));
			}
		}
	...
	public void addStartAnalyisisRacesStoreListener(
			StartAnalysisRacesStoreListener startAnalyisisRacesStoreListener) {
		listeners.add(startAnalyisisRacesStoreListener);
	}

	private WindAndAdvantagesInfoForStartLineDTO createStartAnalysisWindAndLineData(
			TrackedRace trackedRace) {
		WindAndAdvantagesInfoForStartLineDTO startAnalysisWindLineInfoDTO = new WindAndAdvantagesInfoForStartLineDTO();
		LineDetails startline = trackedRace.getStartLine(trackedRace
				.getStartTimeReceived());
		startAnalysisWindLineInfoDTO.startLineAdvantageByGeometry = startline
				.getGeometricAdvantage().getMeters();
		startAnalysisWindLineInfoDTO.startLineAdvantageAtPinEnd = startline
				.getAdvantage().getMeters();
		Position portMarkPosition = trackedRace.getOrCreateTrack(
				trackedRace.getStartLine(trackedRace.getStartTimeReceived())
						.getStarboardMarkWhileApproachingLine())
				.getEstimatedPosition(trackedRace.getStartTimeReceived(), /* extrapolate */
						false);
		Wind windAtStart = trackedRace.getWind(portMarkPosition,
				trackedRace.getStartTimeReceived());
		startAnalysisWindLineInfoDTO.windDirectionInDegrees = windAtStart
	...
	public void notifyListernersAboutStartAnalysisDTOChange() {
		for (StartAnalysisRacesStoreListener startAnalyisisRacesStoreListener : listeners) {
			startAnalyisisRacesStoreListener
					.startAnalyisisRacesChanged(startAnalysisDTOListForCompetitorName);
		}
	}
	 ...
	 * */
	private Map<String, List<GPSFixDTO>> getGPSFixDTOListForCompetitorName(
			TrackedRace trackedRace,
			Map<String, Date> fromPerCompetitorIdAsString,
		...
		if (trackedRace != null) {
			for (Competitor competitor : trackedRace.getRace().getCompetitors()) {
				if (fromPerCompetitorIdAsString.containsKey(competitor.getId()
						.toString())) {
					CompetitorDTO competitorDTO = baseDomainFactory
					...
					GPSFixTrack<Competitor, GPSFixMoving> track = trackedRace
							.getTrack(competitor);
					TimePoint fromTimePoint = new MillisecondsTimePoint(
							fromPerCompetitorIdAsString.get(competitorDTO
									.getIdAsString()));
					TimePoint toTimePoint = new MillisecondsTimePoint(
							toPerCompetitorIdAsString.get(competitorDTO
									.getIdAsString()));
					...
					track.lockForRead();
					try {
						Iterator<GPSFixMoving> fixIter = track
								.getFixesIterator(fromTimePoint, /* inclusive */
										true);
						while (fixIter.hasNext()) {
							GPSFixMoving fix = fixIter.next();
							if (fix.getTimePoint().after(fromTimePoint)
									&& fix.getTimePoint().before(toTimePoint)) {
								fixes.add(fix);
							}
						}
					} finally {
					...
					while (fixIter.hasNext()) {
						GPSFixMoving fix = fixIter.next();
						Wind wind = trackedRace.getWind(fix.getPosition(),
								toTimePoint);
						final SpeedWithBearing estimatedSpeed = track
								.getEstimatedSpeed(fix.getTimePoint());
						Tack tack = wind == null ? null : trackedRace.getTack(
								estimatedSpeed, wind, fix.getTimePoint());
						LegType legType = null;
						WindDTO windDTO = wind == null ? null
								: createWindDTOFromAlreadyAveraged(wind,
										toTimePoint);
						GPSFixDTO fixDTO = createGPSFixDTO(fix, estimatedSpeed,
								windDTO, tack, legType, /* extrapolate */
								false);
	...
	private SpeedWithBearingDTO createSpeedWithBearingDTO(
			SpeedWithBearing speedWithBearing) {
		return new SpeedWithBearingDTO(speedWithBearing.getKnots(),
				speedWithBearing.getBearing().getDegrees());
	}

	private GPSFixDTO createGPSFixDTO(GPSFix fix,
			SpeedWithBearing speedWithBearing, WindDTO windDTO, Tack tack,
			LegType legType, boolean extrapolated) {
		return new GPSFixDTO(fix.getTimePoint().asDate(),
				fix.getPosition() == null ? null : new PositionDTO(fix
						.getPosition().getLatDeg(), fix.getPosition()
						.getLngDeg()), speedWithBearing == null ? null
						: createSpeedWithBearingDTO(speedWithBearing), windDTO,
				tack, legType, extrapolated);
	}

	private StartAnalysisCompetitorDTO createStartAnalysisCompetitorDTO(
			TrackedRace trackedRace, int rank, Competitor competitor) {
		StartAnalysisCompetitorDTO startAnalysisCompetitorDTOsForRace = new StartAnalysisCompetitorDTO();
		startAnalysisCompetitorDTOsForRace.competitorDTO = baseDomainFactory
				.convertToCompetitorDTO(competitor);
		startAnalysisCompetitorDTOsForRace.rankingTableEntryDTO = createRankTableEntry(
				trackedRace, rank, competitor);
		List<GPSFixDTO> boatPositions = getOrCreateGPSFixDTOsForCompetitorForTrackedRace(
				trackedRace).get(competitor.getName());
		startAnalysisCompetitorDTOsForRace.gpsFixDTOs = boatPositions;
		return startAnalysisCompetitorDTOsForRace;
	}


	private Map<String, List<GPSFixDTO>> getOrCreateGPSFixDTOsForCompetitorForTrackedRace(
			TrackedRace trackedRace) {
		try {
			if (!gpsFixesPathBeforeStartForCompetitor
					.containsKey(trackedRace)) {
				gpsFixesPathBeforeStartForCompetitor.put(
						trackedRace,
						getGPSFixDTOListForCompetitorName(
								trackedRace,
								createPerCompetitorIdAsString(trackedRace,
										new Date(trackedRace
												.getStartTimeReceived()
	...
	public static List<StartAnalysisDTO> cloneList(List<StartAnalysisDTO> list) {
		List<StartAnalysisDTO> clone = new ArrayList<StartAnalysisDTO>(
				list.size());
	...
	private class Listener extends AbstractRaceChangeListener {
		...
		public void markPassingReceived(Competitor competitor,
			...
			logger.log(Level.INFO,
					"Received Mark Passing " + competitor.getName());
			if (markPassings.iterator().hasNext()) {
				if (getIndexOfMarkPassingMark(markPassings, trackedRace) == 1) {
					logger.log(Level.INFO,
							"Received Mark Passing " + competitor.getName()+" Mark 1");
					getOrCreateAndAddRankingListAtMarkOneForTrackedRace(trackedRace).add(competitor);
					logger.log(Level.INFO,
							"Received Mark Passing " + competitor.getName()+" RankingList Size"+getOrCreateAndAddRankingListAtMarkOneForTrackedRace(trackedRace).size());
					if(getOrCreateAndAddRankingListAtMarkOneForTrackedRace(trackedRace).size() == MINIMUM_NUMBER_OF_COMPETITORS_IN_STARTANALYSISRANKINGTABLE) {
						for(Competitor competitorInRankingList : getOrCreateAndAddRankingListAtMarkOneForTrackedRace(trackedRace)){
							getOrCreateAndAddStartAnalysisDTOListForCompetitorName(competitorInRankingList.getName()).add(createStartAnalysisDTOForCompetitor(competitorInRankingList, trackedRace));
						}
						logger.log(Level.INFO,
								"Received Mark Passing " + competitor.getName()+" StartAnalysisDTOlist for competitor size: "+getOrCreateAndAddStartAnalysisDTOListForCompetitorName(competitor.getName()).size());
						...
						getOrCreateAndAddStartAnalysisDTOListForCompetitorName(competitor.getName()).add(createStartAnalysisDTOForCompetitor(competitor, trackedRace));
						logger.log(Level.INFO,
								"Received Mark Passing " + competitor.getName()+" StartAnalysisDTOlist for competitor size: "+getOrCreateAndAddStartAnalysisDTOListForCompetitorName(competitor.getName()).size());
						notifyListernersAboutStartAnalysisDTOChange();
					}
