...
public class RibDashboardServiceImpl extends RemoteServiceServlet implements RibDashboardService,
    ...
    private StartAnalyisisRacesStore startAnalyisisRacesStore;
    ...
    public RibDashboardServiceImpl() {
        ...
        startAnalyisisRacesStore = new StartAnalyisisRacesStore(getRacingEventService());
        startAnalyisisRacesStore.addStartAnalyisisRacesStoreListener(this);
    ...
    /**
     * <param>leaderboardGroupName</param>
     * is used to retrieve the live running race with the method {@link #getLiveRaceFromLeaderboardName(String)}.
     * <param>competitorName</param>
     * If competitorName is null, the response contains only a list of competitors in the live race.
     * Otherwise the parameter is used to return the right startanalysis for a specific competitor with in
     * the returned {@link RibDashboardRaceInfoDTO}.
     * */
    @Override
        ...
        if (leaderboardGroupName != null) {
            ...
            if (checkIfRaceIsStillRunning(timePointOfRequest, leaderboardGroupName)) {
                if (competitorName == null) {
                    ...
                    fillLiveRaceInfoDTOWithRaceData(lRInfo, timePointOfRequest);
                    fillLiveRaceInfoDTOWithWindBotData(lRInfo, timePointOfRequest);
    ...
    private void fillLiveRaceInfoDTOWithWindBotData(RibDashboardRaceInfoDTO lRInfo, TimePoint timePoint) {
        if (runningRace != null || runningRace.isLive(timePoint) != false) {
            ...
            Iterator<WindSource> windsourcedInRace = runningRace.getWindSources().iterator();
            ...
            WindInfoForRaceDTO windInfoForRaceDTO = getAveragedWindInfo(runningRace.getRaceIdentifier(), new Date(
                    runningRace.getTimePointOfNewestEvent().asMillis() - 10000), new Date(runningRace
                    .getTimePointOfNewestEvent().asMillis()), 1000, windSourcesToDeliver, true);

            Set<WindSource> windSourcesWithWind = windInfoForRaceDTO.windTrackInfoByWindSource.keySet();
            for (WindSource windSource : windSourcesWithWind) {
                WindTrackInfoDTO windTrackInfoDTOForSource = windInfoForRaceDTO.windTrackInfoByWindSource
                        .get(windSource);
                if (windTrackInfoDTOForSource.windFixes != null && windTrackInfoDTOForSource.windFixes.size() > 0) {
                    WindDTO windDTO = windTrackInfoDTOForSource.windFixes.get(windTrackInfoDTOForSource.windFixes
                            .size() - 1);
                    if (windSource.getType() == WindSourceType.EXPEDITION) {
                        ...
                        Pair<Double, Double> windAverages = getUpdatedWindBotAverages(windSource.getId().toString(),
                                windDTO.trueWindSpeedInKnots, windDTO.trueWindBearingDeg);
                        windBotDTO.averageWindSpeedInKts = windAverages.getA();
                        windBotDTO.averageWindDirectionInDegrees = windAverages.getB();
    ...
    private Pair<Double, Double> getUpdatedWindBotAverages(String botId, double windSpeed, double windDirection) {
        Pair<MovingAverage, MovingAverage> windBotAveragesPair = speedAndDirectionAverageForWindBotID.get(botId);
        if (windBotAveragesPair == null) {
            ...
            speedAndDirectionAverageForWindBotID.put(botId, newWindBotAveragesPair);
        ...
        Pair<Double, Double> windAverages = new Pair<Double, Double>(Double.valueOf(speedAverage.getAverage()),
    ...
    public TrackedRace getExistingTrackedRace(RegattaAndRaceIdentifier regattaNameAndRaceName) {
        return getRacingEventService().getExistingTrackedRace(regattaNameAndRaceName);
    }
