...
public class ParallelGroupedNumberPairDataAverageAggregationProcessor
    ...
    protected void storeElement(GroupedDataEntry<Pair> element) {
        if (element.getDataEntry() != null) {
            ...
            final double doubleValueA = ((Number) element.getDataEntry().getA()).doubleValue();
            final double doubleValueB = ((Number) element.getDataEntry().getA()).doubleValue();
            if (aggregate == null) {
                aggregate = new Pair<>(doubleValueA, doubleValueB);
                sumPerKey.put(element.getKey(), aggregate);
            } else {
                sumPerKey.put(element.getKey(), new Pair<>(((Number) element.getDataEntry().getA()).doubleValue() + doubleValueA,((Number) element.getDataEntry().getB()).doubleValue() + doubleValueB));
            }
    ...
    protected Map<GroupKey, AveragePairWithStats<Number>> aggregateResult() {
        ...
        for (Entry<GroupKey, Pair<Number, Number>> sumAggregationEntry : sumPerKey.entrySet()) {
            ...
            result.put(key, new AveragePairWithStatsImpl<Number>(new Pair<>(sumAggregationEntry.getValue().getA().doubleValue() / elementAmountPerKey.get(key).get(),sumAggregationEntry.getValue().getB().doubleValue() / elementAmountPerKey.get(key).get()),
