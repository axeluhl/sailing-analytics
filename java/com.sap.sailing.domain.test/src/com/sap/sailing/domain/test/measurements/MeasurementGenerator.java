...
public class MeasurementGenerator {
    ...
    public void writeMeasurementsToFile() throws IOException {
        Writer w = new BufferedWriter(new FileWriter(new File(reportFileName)));
        w.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        w.write("<testsuite  name=\""+getSuiteName()+" tests=\""+getNumberOfTests()+"\" >\n");
        for (MeasurementCase measurementCase : getCases()) {
            w.write("<testcase   name=\""+measurementCase.getName()+"\" status=\"run\" time=\"0\" classname=\""+measurementCase.getName()+"\">\n");
            w.write("<system-out>\n");
            for (Measurement measurement : measurementCase.getMeasurements()) {
                w.write("&lt;measurement&gt;&lt;name&gt;"+measurement.getName()+"&lt;/name&gt;&lt;value&gt;"+measurement.getValue()+
                        "&lt;/value&gt;&lt;/measurement&gt;");
            }
            w.write("</system-out>\n");
            w.write("</testcase>\n");
