...
/**
 * A leaderboard essentially consists of a table widget that in its columns
 * displays the entries.
 * 
 * @author Axel Uhl (D043530)
 * 
 */
public class LeaderboardPanel extends FormPanel implements TimeListener,
		PlayStateListener {
	 ...
	 * obtain the leaderboard contents} from the server. It may change in case
	 * the leaderboard is renamed.
	 */
	...
	/**
	 * The delay with which the timer shall work. Before the timer is resumed,
	 * the delay is set to this value.
	 */
	private long delayInMilliseconds;

	/**
	 * This anchor's HTML holds the image tag for the play/pause button that
	 * needs to be updated when the {@link #timer} changes its playing state
	 */
	...
	private class SettingsClickHandler implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			new LeaderboardSettingsPanel(
					Collections.unmodifiableList(selectedLegDetails),
					Collections.unmodifiableList(selectedRaceDetails),
					timer.getDelayBetweenAutoAdvancesInMilliseconds(),
					stringConstants.leaderboardSettings(),
					stringConstants.selectLegDetails(), stringConstants.ok(),
					stringConstants.cancel(),
					new Validator<LeaderboardSettingsPanel.Result>() {
						@Override
						public String getErrorMessage(
								LeaderboardSettingsPanel.Result valueToValidate) {
							if (valueToValidate.getLegDetailsToShow().isEmpty()) {
								return stringConstants
										.selectAtLeastOneLegDetail();
							} else if (valueToValidate
									.getDelayBetweenAutoAdvancesInMilliseconds() < 1000) {
								return stringConstants
										.chooseUpdateIntervalOfAtLeastOneSecond();
							} else {
								return null;
							}
						}
						...
						public void onSuccess(
								LeaderboardSettingsPanel.Result result) {
							List<ExpandableSortableColumn<?>> columnsToExpandAgain = new ArrayList<ExpandableSortableColumn<?>>();
							for (int i = 0; i < getLeaderboardTable()
									.getColumnCount(); i++) {
								Column<LeaderboardRowDAO, ?> c = getLeaderboardTable()
										.getColumn(i);
								if (c instanceof ExpandableSortableColumn<?>) {
									...
									if (expandableSortableColumn.isExpanded()) {
										...
										// child columns
										expandableSortableColumn
												.toggleExpansion();
										columnsToExpandAgain
												.add(expandableSortableColumn);
									}
								}
							}
	...
	protected class CompetitorColumn extends
			SortableColumn<LeaderboardRowDAO, String> {
			...
			return new Comparator<LeaderboardRowDAO>() {
				...
				public int compare(LeaderboardRowDAO o1, LeaderboardRowDAO o2) {
					return Collator.getInstance().compare(
							getLeaderboard().getDisplayName(o1.competitor),
	 ...
	 * 
	 */
	private class SailIDColumn extends
			SortableColumn<LeaderboardRowDAO, String> {
		...
		public Comparator<LeaderboardRowDAO> getComparator() {
			return new Comparator<LeaderboardRowDAO>() {
				@Override
				public int compare(LeaderboardRowDAO o1, LeaderboardRowDAO o2) {
					return o1.competitor.sailID == null ? o2.competitor.sailID == null ? 0
							: -1
							: o2.competitor.sailID == null ? 1 : Collator
									.getInstance().compare(
											o1.competitor.sailID,
		...
		@Override
		public void render(Context context, LeaderboardRowDAO object,
				SafeHtmlBuilder sb) {
			sb.appendHtmlConstant("<img title=\""
					+ object.competitor.countryName + "\" src=\""
		...
		private String getFlagURL(String twoLetterIsoCountryCode) {
			return "/images/flags/" + twoLetterIsoCountryCode.toLowerCase()
					+ ".png";
		}
	...
	/**
	 * Displays net/total points and possible max-points reasons based on a
	 * {@link LeaderboardRowDAO} and a race name and makes the column sortable
	 * by the total points.
	 * 
	 * @author Axel Uhl (D043530)
	 * 
	...
	protected abstract class RaceColumn<C> extends ExpandableSortableColumn<C> {
		...
		public RaceColumn(String raceName, boolean medalRace,
				boolean enableExpansion, Cell<C> cell, String headerStyle,
				String columnStyle) {
			super(LeaderboardPanel.this, enableExpansion, cell,
					stringConstants, LEG_COLUMN_HEADER_STYLE, LEG_COLUMN_STYLE,
					selectedRaceDetails);
		...
		/**
		 * Displays a combination of total points and maxPointsReason in bold,
		 * transparent, strike-through, depending on various criteria. Here's
		 * how:
		 * 
		 * <pre>
		...
		public Comparator<LeaderboardRowDAO> getComparator() {
			if (medalRace) {
				...
				return new Comparator<LeaderboardRowDAO>() {
					@Override
					public int compare(LeaderboardRowDAO o1,
							LeaderboardRowDAO o2) {
						boolean ascending = isSortedAscendingForThisColumn(getLeaderboardPanel()
								.getLeaderboardTable());
						LeaderboardEntryDAO o1Entry = o1.fieldsByRaceName
								.get(raceName);
						LeaderboardEntryDAO o2Entry = o2.fieldsByRaceName
								.get(raceName);
						return (o1Entry == null || o1Entry.netPoints == 0) ? (o2Entry == null || o2Entry.netPoints == 0) ? 0
								: ascending ? 1 : -1
								: (o2Entry == null || o2Entry.netPoints == 0) ? ascending ? -1
										: 1
	...
	public static DetailColumnType[] getAvailableRaceDetailColumnTypes() {
		return new DetailColumnType[] {
				DetailColumnType.RACE_AVERAGE_SPEED_OVER_GROUND_IN_KNOTS,
				DetailColumnType.RACE_DISTANCE_TRAVELED,
				DetailColumnType.RACE_GAP_TO_LEADER_IN_SECONDS };
	}

	private class TextRaceColumn extends RaceColumn<String> {
		/**
		 * Remembers the leg columns; <code>null</code>-padded, if
		 * {@link #getLegColumn(int)} asks for a column index not yet existing.
		 * It is important to remember the columns because column removal
		 * happens based on identity.
		 */
		private final List<LegColumn> legColumns;

		public TextRaceColumn(String raceName, boolean medalRace,
				boolean expandable, String headerStyle, String columnStyle) {
		...
		@Override
		protected void ensureExpansionDataIsLoaded(
				final Runnable callWhenExpansionDataIsLoaded) {
		...
		protected Map<DetailColumnType, SortableColumn<LeaderboardRowDAO, ?>> getDetailColumnMap(
				...
				StringConstants stringConstants, String detailHeaderStyle,
			...
			result.put(
					DetailColumnType.RACE_DISTANCE_TRAVELED,
					new FormattedDoubleLegDetailColumn(stringConstants
							.distanceInMeters(),
							new RaceDistanceTraveledInMeters(), 1,
							getLeaderboardPanel().getLeaderboardTable(),
							LEG_COLUMN_HEADER_STYLE, LEG_COLUMN_STYLE));
			result.put(
					DetailColumnType.RACE_AVERAGE_SPEED_OVER_GROUND_IN_KNOTS,
					new FormattedDoubleLegDetailColumn(stringConstants
							.averageSpeedInKnots(),
							new RaceAverageSpeedInKnots(), 1,
							getLeaderboardPanel().getLeaderboardTable(),
							LEG_COLUMN_HEADER_STYLE, LEG_COLUMN_STYLE));
			result.put(
					DetailColumnType.RACE_GAP_TO_LEADER_IN_SECONDS,
					new FormattedDoubleLegDetailColumn(stringConstants
							.gapToLeaderInSeconds(),
							new RaceGapToLeaderInSeconds(), 1,
							getLeaderboardPanel().getLeaderboardTable(),
							LEG_COLUMN_HEADER_STYLE, LEG_COLUMN_STYLE));
			return result;
		}
		...
		protected Iterable<SortableColumn<LeaderboardRowDAO, ?>> getDirectChildren() {
			...
			for (SortableColumn<LeaderboardRowDAO, ?> column : super
					.getDirectChildren()) {
				result.add(column);
			}
			if (isExpanded()) {
		...
		private LegColumn getLegColumn(int legNumber) {
			LegColumn result;
			if (legColumns.size() > legNumber
					&& legColumns.get(legNumber) != null) {
				result = legColumns.get(legNumber);
			} else {
				result = new LegColumn(LeaderboardPanel.this, getRaceName(),
						legNumber, stringConstants,
						Collections.unmodifiableList(selectedLegDetails),
						LEG_COLUMN_HEADER_STYLE, LEG_COLUMN_STYLE,
		...
		private class RaceAverageSpeedInKnots implements LegDetailField<Double> {
			...
			public Double get(LeaderboardRowDAO row) {
				Double result = null;
				LeaderboardEntryDAO fieldsForRace = row.fieldsByRaceName
						.get(getRaceName());
				if (fieldsForRace != null && fieldsForRace.legDetails != null) {
					...
					if (timeInMilliseconds != 0) {
						result = distanceTraveledInMeters
								/ (double) timeInMilliseconds * 1000 * 3600
								/ 1852;
					}
				}
				return result;
		 ...
		 * @author Axel Uhl (D043530)
		 */
		private class RaceDistanceTraveledInMeters implements
				LegDetailField<Double> {
			@Override
			public Double get(LeaderboardRowDAO row) {
				Double result = null;
				LeaderboardEntryDAO fieldsForRace = row.fieldsByRaceName
		 ...
		 * @author Axel Uhl (D043530)
		 */
		private class RaceGapToLeaderInSeconds implements
				LegDetailField<Double> {
			@Override
			public Double get(LeaderboardRowDAO row) {
				Double result = null;
				LeaderboardEntryDAO fieldsForRace = row.fieldsByRaceName
						.get(getRaceName());
				if (fieldsForRace != null && fieldsForRace.legDetails != null) {
					LegEntryDAO lastLegDetail = fieldsForRace.legDetails
							.get(fieldsForRace.legDetails.size() - 1);
	 ...
	 * 
	 */
	private class TotalsColumn extends
			SortableColumn<LeaderboardRowDAO, String> {
		private final String columnStyle;
	...
	protected class CarryColumn extends
			SortableColumn<LeaderboardRowDAO, String> {
		public CarryColumn() {
			super(new TextCell());
			setSortable(true);
		...
		@Override
		public String getValue(LeaderboardRowDAO object) {
			return object.carriedPoints == null ? "" : ""
					+ object.carriedPoints;
		}
	...
	private class RankColumn extends SortableColumn<LeaderboardRowDAO, String> {
		...
		public Comparator<LeaderboardRowDAO> getComparator() {
			return new Comparator<LeaderboardRowDAO>() {
				@Override
				public int compare(LeaderboardRowDAO o1, LeaderboardRowDAO o2) {
					return getLeaderboard().getRank(o1.competitor)
							- getLeaderboard().getRank(o2.competitor);
				}
			};
		}
	...
	public LeaderboardPanel(SailingServiceAsync sailingService,
		...
		this.selectedLegDetails
				.add(DetailColumnType.AVERAGE_SPEED_OVER_GROUND_IN_KNOTS);
		...
		LeaderboardTableResources resources = GWT
				.create(LeaderboardTableResources.class);
		RACE_COLUMN_HEADER_STYLE = resources.cellTableStyle()
				.cellTableRaceColumnHeader();
		LEG_COLUMN_HEADER_STYLE = resources.cellTableStyle()
				.cellTableLegColumnHeader();
		LEG_DETAIL_COLUMN_HEADER_STYLE = resources.cellTableStyle()
		...
		LEG_DETAIL_COLUMN_STYLE = resources.cellTableStyle()
		...
		getLeaderboardTable().setSelectionModel(
				new MultiSelectionModel<LeaderboardRowDAO>() {
				});
		...
		Anchor settingsAnchor = new Anchor(
				new SafeHtmlBuilder()
						.appendHtmlConstant(
								"<img class=\"linkNoBorder\" src=\"/images/settings.png\"/>")
						.toSafeHtml());
		settingsAnchor.setTitle(stringConstants.settings());
		settingsAnchor
				.addClickHandler(new SettingsClickHandler(stringConstants));
	...
	private SafeHtml getPlayPauseImgHtml(boolean playing) {
		return new SafeHtmlBuilder().appendHtmlConstant(
				"<img class=\"linkNoBorder\" src=\"/images/"
						+ (playing ? "pause" : "play") + "_16.png\"/>")
				.toSafeHtml();
	}
	...
	/**
	 * adds the <code>column</code> to the right end of the
	 * {@link #getLeaderboardTable() leaderboard table} and sets the column
	 * style according to the {@link SortableColumn#getColumnStyle() column's
	 * style definition}.
	 */
	protected void addColumn(SortableColumn<LeaderboardRowDAO, ?> column) {
		...
		if (columnStyle != null) {
			getLeaderboardTable().addColumnStyleName(
					getLeaderboardTable().getColumnCount() - 1, columnStyle);
		}
	}

	protected void insertColumn(int beforeIndex,
			SortableColumn<LeaderboardRowDAO, ?> column) {
		// remove column styles of those columns whose index will shift right by
		// one:
		removeColumnStyles(beforeIndex);
		getLeaderboardTable().insertColumn(beforeIndex, column,
				column.getHeader());
	...
	/**
	 * removes the column specified by <code>columnIndex</code> from the
	 * {@link #getLeaderboardTable() leaderboard table} and fixes the column
	 * styles again (see {@link #addColumnStyles(int)}).
	 */
	...
	private Collection<String> getNamesOfExpandedRaces() {
		...
		for (int i = 0; i < getLeaderboardTable().getColumnCount(); i++) {
			Column<LeaderboardRowDAO, ?> column = getLeaderboardTable()
					.getColumn(i);
	...
	private void updateLeaderboard(LeaderboardDAO leaderboard) {
		...
		if (leaderboard != null) {
			...
			for (int i = 0; i < getLeaderboardTable().getColumnCount(); i++) {
				SortableColumn<?, ?> c = (SortableColumn<?, ?>) getLeaderboardTable()
						.getColumn(i);
			...
			if (comparator != null) {
				...
				// table header so
				Collections.sort(getData().getList(), getRankColumn()
						.getComparator());
	...
	private Comparator<LeaderboardRowDAO> getComparatorForSelectedSorting() {
		...
		if (getLeaderboardTable().getColumnSortList().size() > 0) {
			ColumnSortInfo columnSortInfo = getLeaderboardTable()
					.getColumnSortList().get(0);
	...
	/**
	 * Existing and matching race columns may still need to be removed,
	 * re-created and inserted because the "tracked" property may have changed,
	 * changing the columns expandability.
	 */
	private void createMissingAndAdjustExistingRaceColumns(
			LeaderboardDAO leaderboard) {
		...
		for (String race : raceList) {
			...
			for (int i = 0; !foundRaceColumn
					&& i < getLeaderboardTable().getColumnCount(); i++) {
				Column<LeaderboardRowDAO, ?> c = getLeaderboardTable()
						.getColumn(i);
				if (c instanceof RaceColumn) {
					...
					if (raceColumn.getRaceName().equals(race)) {
						...
						if (raceColumn.isExpansionEnabled() != leaderboard
								.raceIsTracked(race)) {
							if (raceColumn.isExpanded()) {
								raceColumn.toggleExpansion(); // remove children
																// from table
							}
						}
						int columnIndex = getLeaderboardTable().getColumnIndex(
								raceColumn);
						int indexRaceColumn = 0;
						for (int j = 0; j < raceList.size(); j++) {
							if (raceList.get(j)
									.equals(raceColumn.getRaceName()))
								indexRaceColumn = j;
						}
						removeColumn(columnIndex);
						insertColumn(
								positionRaceColumn[indexRaceColumn],
								createRaceColumn(race,
										leaderboard.raceIsMedalRace(race),
										leaderboard.raceIsTracked(race)));
					}
				}
			}
			if (!foundRaceColumn) {
				addRaceColumn(createRaceColumn(race,
						leaderboard.raceIsMedalRace(race),
						leaderboard.raceIsTracked(race)));
			}
		}
	...
	protected RaceColumn<?> createRaceColumn(String raceName,
			boolean isMedalRace, boolean isTracked) {
		return new TextRaceColumn(raceName, isMedalRace, isTracked,
				RACE_COLUMN_HEADER_STYLE, RACE_COLUMN_STYLE);
	}
	...
	private void removeUnusedRaceColumns(LeaderboardDAO leaderboard) {
		...
		for (int i = 0; i < getLeaderboardTable().getColumnCount(); i++) {
			 ...
			 * !leaderboard.raceNamesAndMedalRaceAndTracked.keySet().contains(
			 * ((RaceColumn<?>) c).getRaceName()))) { columnsToRemove.add(c); }
			 */
			if (c instanceof RaceColumn
					&& (leaderboard == null || !leaderboard
							.raceListContains(((RaceColumn<?>) c).getRaceName()))) {
	...
	/**
	 * If the last column is the totals column, remove it. Add the race column
	 * as the last column.
	 */
	private void addRaceColumn(RaceColumn<?> raceColumn) {
		if (getLeaderboardTable().getColumn(
				getLeaderboardTable().getColumnCount() - 1) instanceof TotalsColumn) {
		...
		if (getLeaderboardTable().getColumnCount() == RANK_COLUMN_INDEX) {
			...
			if (!(getLeaderboardTable().getColumn(RANK_COLUMN_INDEX) instanceof RankColumn)) {
				throw new RuntimeException(
						"The first column must always be the rank column but it was of type "
								+ getLeaderboardTable()
										.getColumn(RANK_COLUMN_INDEX)
										.getClass().getName());
			}
		}
	}
	...
	private void ensureSailIDAndCompetitorColumn() {
		if (getLeaderboardTable().getColumnCount() <= SAIL_ID_COLUMN_INDEX) {
			...
			if (!(getLeaderboardTable().getColumn(SAIL_ID_COLUMN_INDEX) instanceof SailIDColumn)) {
				throw new RuntimeException(
						"The second column must always be the sail ID column but it was of type "
								+ getLeaderboardTable()
										.getColumn(SAIL_ID_COLUMN_INDEX)
										.getClass().getName());
			}
		}
	}
	...
	/**
	 * If the <code>leaderboard</code> {@link LeaderboardDAO#hasCarriedPoints
	 * has carried points} and if column #1 (second column, right of the
	 * competitor column) does not exist or is not of type {@link CarryColumn},
	 * all columns starting from #1 will be removed and a {@link CarryColumn}
	 * will be added. If the leaderboard has no carried points but the display
	 * still shows a carry column, the column is removed.
	 */
	...
	public void playStateChanged(boolean isPlaying) {
		...
		playPause.setTitle(isPlaying ? stringConstants.pauseAutomaticRefresh()
				: stringConstants.autoRefresh());
	}
