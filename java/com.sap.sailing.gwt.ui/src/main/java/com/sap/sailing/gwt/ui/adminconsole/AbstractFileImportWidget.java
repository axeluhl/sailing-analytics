...
public abstract class AbstractFileImportWidget extends Composite {
    ...
    protected AbstractFileImportWidget(String formActionUrl, String importButtonText,
        ...
        importButtonUi.setText(importButtonText);
    ...
    void onFileImportComplete(SubmitCompleteEvent event) {
        List<String> uuids = Arrays.asList(event.getResults().split("\\n"));
        sailingService.getTrackFileImportDeviceIds(uuids, new AsyncCallback<List<TrackFileImportDeviceIdentifierDTO>>() {
            @Override
