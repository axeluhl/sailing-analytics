...
public class EventRegattaSchedule extends Composite {
    ...
    public EventRegattaSchedule(EventDTO event, Timer timerForClientServerOffset, EventPageNavigator pageNavigator) {
        ...
        regattaHeader = new EventRegattaHeader(event);
        
        EventRegattaScheduleResources.INSTANCE.css().ensureInjected();
        initWidget(uiBinder.createAndBindUi(this));
    }
    
    public void setRacesFromRegatta(RegattaDTO regatta, StrippedLeaderboardDTO regattaLeaderboard) {
        regattaHeader.setData(regattaLeaderboard.displayName != null ? regattaLeaderboard.displayName : regattaLeaderboard.name, "???", 100, true);
        
        for(SeriesDTO series: regatta.series) {
            EventRegattaScheduleSeries eventRegattaScheduleSeries = new EventRegattaScheduleSeries(regattaLeaderboard, series, timerForClientServerOffset); 
