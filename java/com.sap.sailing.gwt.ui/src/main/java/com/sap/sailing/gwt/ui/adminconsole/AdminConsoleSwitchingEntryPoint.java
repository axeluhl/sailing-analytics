...
import com.sap.sailing.gwt.ui.adminconsole.shared.app.MobileSupport;
 ...
 * </ul>
 */
public class AdminConsoleSwitchingEntryPoint implements EntryPoint {
    private static Logger LOG = Logger.getLogger(AdminConsoleSwitchingEntryPoint.class.getName());
    private static final String SAP_ADMINCONSOLE_MOBILE = "sap_adminconsole_mobile";

    private final PlaceHistoryMapper hisMap = GWT.create(AdminConsolePlaceHistoryMapper.class);
    ...
    public void onModuleLoad() {
        ...
        String userWantsMobileUi = Cookies.getCookie(SAP_ADMINCONSOLE_MOBILE);
    ...
    public static void switchToDesktop() {
        if (DeviceDetector.isDesktop()) {
            Cookies.removeCookie(SAP_ADMINCONSOLE_MOBILE);
        } else {
            // only force "not mobile" on mobile devices
            Cookies.setCookie(SAP_ADMINCONSOLE_MOBILE, "false");
        }
        reloadApp();
    }
    ...
    public static void switchToMobile() {
        if (DeviceDetector.isMobile()) {
            ...
            Cookies.setCookie(SAP_ADMINCONSOLE_MOBILE, "true");
        }
        reloadApp();
    }
    ...
    private void startMobile() {
        ...
        new AdminConsoleMobileEntryPoint().onModuleLoad();
    }

    private void startDesktop() {
        configureDesktopHeader();
        new AdminConsoleDesktopEntryPoint().onModuleLoad();
    }
