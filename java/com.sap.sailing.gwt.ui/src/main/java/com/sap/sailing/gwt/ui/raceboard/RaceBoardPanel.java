...
/**
 * A panel showing a list of components visualizing a race from the regattas announced by calls to
 * {@link #fillRegattas(List)}. The race selection is provided by a {@link RaceSelectionProvider} for which this is a
 * {@link RaceSelectionChangeListener listener}. {@link RaceIdentifier}-based race selection changes are converted to
 * {@link RaceDTO} objects using the {@link #racesByIdentifier} map maintained during {@link #fillRegattas(List)}. The
 * race selection provider is expected to be single selection only.
 * 
...
public class RaceBoardPanel extends FormPanel implements RegattaDisplayer, RaceSelectionChangeListener {
    ...
    public RaceBoardPanel(SailingServiceAsync sailingService, MediaServiceAsync mediaService, UserDTO theUser,
            ...
            UserAgentDetails userAgent, RaceBoardViewModes viewMode, RaceTimesInfoProvider raceTimesInfoProvider) {
    ...
    private void createOneScreenView(String leaderboardName, String leaderboardGroupName, FlowPanel mainPanel) {
        ...
        RaceMap raceMap = new RaceMap(sailingService, asyncActionsExecutor, errorReporter, timer,
                competitorSelectionModel, stringMessages);
        raceTimesInfoProvider.addRaceTimesInfoProviderListener(raceMap);
        raceMap.onRaceSelectionChange(Collections.singletonList(selectedRaceIdentifier));

        List<Component<?>> components = new ArrayList<Component<?>>();

        competitorChart = new MultiChartPanel(sailingService, asyncActionsExecutor, competitorSelectionModel,
                raceSelectionProvider, timer, timeZoomModel, stringMessages, errorReporter, true, true);
     ...
     * Displays or hides the leaderboard, if in <code>ONESCREEN</code> view mode.<br />
     * <br />
     * 
     * The race board should be completely rendered before this method is called, or a few exceptions could be thrown.
     * 
     * @param visible
     *            <code>true</code> if the leaderboard shall be open/visible
     */
     ...
     * Displays or hides the wind chart, if in <code>ONESCREEN</code> view mode.<br />
     * <br />
     * 
     * The race board should be completely rendered before this method is called, or a few exceptions could be thrown.
     * 
     * @param visible
     *            <code>true</code> if the wind chart shall be open/visible
     */
     ...
     * Displays or hides the competitor chart, if in <code>ONESCREEN</code> view mode.<br />
     * <br />
     * 
     * The race board should be completely rendered before this method is called, or a few exceptions could be thrown.
     * 
     * @param visible
     *            <code>true</code> if the competitor chart shall be open/visible
     */
     ...
     * 
     * @param scrollOffset
     *            The new scrolling offset. <b>Only</b> the absolute value will be used.
     */
