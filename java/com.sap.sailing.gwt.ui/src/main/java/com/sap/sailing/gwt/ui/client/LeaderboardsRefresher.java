...
public interface LeaderboardsRefresher<T extends StrippedLeaderboardDTO> {
     ...
     * changed the leaderboard collection and has already updated its sequence of {@link StrippedLeaderboardDTO}s. This
     * does not necessarily create consistency with the leaderboards known by the server at that time, but at least all
     * areas of the UI that have registered as {@link LeaderboardsDisplayer} will show an equal sequence of
     * leaderboards.
     * 
     * @param origin
     *            will not receive a call to its {@link LeaderboardsDisplayer#fillLeaderboards(Iterable)}, assuming that
