 ...
 * <p>
 * 
 * Since {@link AverageWithStats} itself is a generic type, another {@link AbstractNumericDataProvider} is needed
 * to map the values to be displayed (other than count which is already guaranteed to be of numeric type) to a
 * numeric type. 
 * 
 * @author Axel Uhl (d043530)
 *
 */
public class AverageWithStatsDataProvider extends AbstractNumericDataProvider<AverageWithStats<? extends Serializable>> {
     ...
     * {@link DataProvidersPrecedenceList#selectCurrentDataProvider(QueryResultDTO)} method to determine the
     * most appropriate inner provider to use for the result.
     */
     ...
     */
    @Override
    protected Function<AverageWithStats<? extends Serializable>, Number> getMapping(QueryResultDTO<?> result, String dataKey) {
        final AbstractNumericDataProvider<? extends Serializable> provider = getProviderFromGenericResult(result);
