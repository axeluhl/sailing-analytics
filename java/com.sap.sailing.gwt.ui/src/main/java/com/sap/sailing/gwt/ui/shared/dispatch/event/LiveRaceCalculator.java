...
final class LiveRaceCalculator implements RaceCallback {
    ...
    public ResultWithTTL<LiveRacesDTO> getResult() {
        ...
        for(LiveRaceDTO race : result.getRaces()) {
            if(race.getState() == RaceViewState.RUNNING) {
                ttl = Math.min(ttl, 1000 * 30);
            } else if(race.getState() == RaceViewState.SCHEDULED) {
            ...
            } else if(race.getState() == RaceViewState.POSTPONED || race.getState() == RaceViewState.CANCELED) {
