...
/**
 *  A google map overlay based on a HTML5 canvas for drawing a wind field.
 *  The overlay covers the whole map and displays the wind objects inside it. 
...
public class WindFieldCanvasOverlay extends CanvasOverlay {
    ...
    private void drawArrow(PositionDTO position, double angle, double length, double weight) {
        ...
        Point canvasPositionInPx = getMap().convertLatLngToDivPixel(LatLng.newInstance(position.latDeg,position.lngDeg));
