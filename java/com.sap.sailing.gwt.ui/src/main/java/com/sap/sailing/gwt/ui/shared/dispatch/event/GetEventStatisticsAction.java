...
public class GetEventStatisticsAction implements Action<ResultWithTTL<EventStatisticsDTO>> {
    ...
    public ResultWithTTL<EventStatisticsDTO> execute(DispatchContext context) {
                    ...
                    for (Competitor competitor : trackedRace.getRace().getCompetitors()) {
                        ...
                        if (trackedRace.hasStarted(now)) {
                            ...
                            if (lastMarkPassingBeforeNow != null) {
                                // competitor has started and has at least one mark passing before now; compute distance traveled
                                // up to that mark passing, increasing likelihood of cache hits as compared to using "now" as
                                // the query time point
                                totalDistanceTraveled = totalDistanceTraveled.add(
                                        competitorTrack.getDistanceTraveled(trackedRace.getStartOfRace(), lastMarkPassingBeforeNow.getTimePoint()));
                                Pair<GPSFixMoving, Speed> competitorMaxSpeed = competitorTrack.getMaximumSpeedOverGround(trackedRace.getStartOfRace(), lastMarkPassingBeforeNow.getTimePoint());
        ...
        return new ResultWithTTL<EventStatisticsDTO>(1000 * 60 * 5, new EventStatisticsDTO(regattas, competitors,
                races, trackedRaces, numberOfGPSFixes, numberOfWindFixes, maxSpeed));
    }
}
