...
public class MediaSelectionDialog implements CloseHandler<PopupPanel> {
    ...
    public void show(Collection<MediaTrack> videoTracks, Collection<MediaTrack> audioTracks, UIObject popupLocation) {
        Panel grid = new VerticalPanel();
        if (!audioTracks.isEmpty()) {
            grid.add(createAudioHeader());
            grid.add(createAudioItem(null));
            for (MediaTrack audioTrack : audioTracks) {
                grid.add(createAudioItem(audioTrack));
            }
            for (MediaTrack audioTrack : videoTracks) {
                grid.add(createAudioItem(audioTrack));
            }
        }
        if (!videoTracks.isEmpty()) {
            ...
            for (MediaTrack videoTrack : videoTracks) {
                grid.add(createVideoItem(videoTrack));
            }
        }
    ...
    private Widget createVideoItem(final MediaTrack videoTrack) {
    ...
    private Widget createAudioItem(final MediaTrack audioTrack) {
