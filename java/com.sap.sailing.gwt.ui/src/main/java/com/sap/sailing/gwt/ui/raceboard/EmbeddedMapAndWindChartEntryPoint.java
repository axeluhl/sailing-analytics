...
public class EmbeddedMapAndWindChartEntryPoint extends AbstractSailingReadEntryPoint implements ProvidesLeaderboardRouting {
    ...
    private void createEmbeddedMap(final RegattaAndRaceIdentifier selectedRaceIdentifier,
        ...
        createRaceBoardInOneScreenMode(raceMap, windChart, paywallResolver);
    ...
    private void createRaceBoardInOneScreenMode(final RaceMap raceMap, final WindChart windChart, PaywallResolver paywallResolver) {
        final TouchSplitLayoutPanel panel = new TouchSplitLayoutPanel(/* horizontal splitter width */ 3, /* vertical splitter height */ 25, paywallResolver);
        if (windChart != null) {
