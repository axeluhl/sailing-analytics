    ...
    private CaptionPanel createExpeditionAllInOneImportPanel() {
        ...
        formPanel.addSubmitCompleteHandler(event -> {
            ...
            new ExpeditionAllInOneAfterImportHandler(response.getEventId(), response.getRegattaName(),
                    response.getLeaderboardName(), response.getRaceName(), response.getRaceColumnName(),
                    response.getFleetName(), response.getGpsDeviceIds(), response.getSensorDeviceIds(),
                    response.getSensorFixImporterType(), sailingService, errorReporter, stringMessages);
        });
        ...
        final Button uploadButton = new Button(stringMessages.upload(), (ClickHandler) event -> formPanel.submit());
