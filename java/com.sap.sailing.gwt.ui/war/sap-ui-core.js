// This file has been generated by the Phoenix CreateModule Ant-Task
/*!
...
if ( window.attachEvent && !window.addEventListener ) {
	...
	if (false) { // actually this is not required in Phoenix-supported browsers; disable for further central performance measurements   window.location.search && window.location.search.toLowerCase().indexOf("sap-ui-perf=true") == -1) {
...
(function() {
	...
	var oCfgData = _window["sap-ui-config"] = (function() {
		...
		if ( oScriptTag ) {
			...
			jQuery.each(oScriptTag.attributes, function(i, attr) {
				...
				if ( m && m[1] !== 'config' ) {
					oCfg[m[1].toLowerCase()] = attr.value;
				}
			});
		}
	...
	var instancePrefix = "";
	if ( window.top ) {
		if ( !window.top["sap-ui-instanceCount"] ) {
			window.top["sap-ui-instanceCount"] = 0;
		}
		window.top["sap-ui-instanceCount"] = window.top["sap-ui-instanceCount"]+1;
		instancePrefix = "[" + window.top["sap-ui-instanceCount"] + "] ";
	}
	*/

	/**
	 * Root Namespace for the jQuery plugin provided by SAP AG.
	 *
	...
	/**
	 * Dummy implementation of a log namespace
	 * @public
	 * @static
	 */
	jQuery.sap.log = {};
	/**
	 * Dummy implementation of a fatal error to be docked to a debug.js for implementation.
	 *
	 * @public
	 * @static
	 */
	jQuery.sap.log.fatal = function(sMessage, sDetails, sComponent) {
		throw new Error(sMessage);
	};

	/**
	 * Dummy implementation of an error to be docked to a debug.js for implementation.
	 *
	 * @public
	 * @static
	 */
	jQuery.sap.log.error = function(sMessage, sDetails, sComponent) {
		if ( _window.console && console.error ) {
	...
	/**
	 * Dummy implementation of a warning to be docked to a debug.js for implementation.
	 *
	 * @public
	 * @static
	 */
	jQuery.sap.log.warning = function(sMessage, sDetails, sComponent) {
		if ( _window.console && console.warning ) {
	...
	/**
	 * Dummy implementation of an info message to be docked to a debug.js for implementation.
	 *
	 * @public
	 * @static
	 */
	jQuery.sap.log.info = function(sMessage, sDetails, sComponent) {
		if ( _window.console && console.info ) {
	...
	/**
	 * Dummy implementation of a debug message to be docked to a debug.js for implementation.
	 *
	 * @public
	 * @static
	 */
	jQuery.sap.log.debug = function(sMessage, sDetails, sComponent) {
		if ( _window.console && (console.debug || console.info) ) {
			(console.debug || console.info)(sMessage);
		}
		return this;
	};

	/**
	 * Dummy implementation of a trace to be docked to a debug js for implementation.
	 *
	 ...
	 * @static
	 */
	jQuery.sap.log.trace = function(sMessage, sDetails, sComponent) {
		return this;
	};
	...
	/**
	 * Dummy implementation of an assert to be docked to a debug js for implementation.
	 *
	 * <b>Note:</b> Calls to this method might be removed when the JavaScript code
	 *              is optimized during build.
	 *
	...
	jQuery.sap.assert = function(bResult, sMessage) {
		// activating the following line reveals quite some open topics :-(
		// if( window["console"] && console.assert ) console.assert(bResult, sMessage);
		return this;
	};
	 ...
	 *
	 *    'sap.com'  ->  'http://www.sap.com/phoenix/resources/'
	 *
	 ...
	 * @static
	 */
	jQuery.sap.require = function require(sModuleName) {
		...
		if ( _verbose ) {
			jQuery.sap.log.debug(sLogPrefix + "sap.require '" + sModuleName + "'");
		}
		...
		mod.url = _getModulePath(sType ? sModuleName.substring(0, sModuleName.length - sType.length) : sModuleName, sType + '.js');
		if ( _verbose ) {
		...
		if ( typeof data !== "undefined" ) {
			try {
				...
				if (_window.execScript) {
					try {
						data && _window.execScript(data); // execScript fails if data is empty
					} catch (e) {
						_execStack.pop();
	...
	/**
	 * Includes the script (via &lt;script&gt;-tag) specified via <code>sUrl</code>.<br/>
	 * If there is call to <code>includeScript</code> providing the sId of an already included script,
	 * the existing script element will be replaced.
	 *
	...
	jQuery.sap.includeScript = function includeScript(sUrl, sId){
		// TODO: Verify that function works correct! Tested correctly!
		var oOldScript, bReplace;
		if((sId && (oOldScript = jQuery.sap.domById(sId)) && (bReplace = oOldScript.tagName === "SCRIPT")) || window.document.body) {
			...
			if(bReplace) {
				jQuery(oOldScript).replaceWith(oScript);
			} else {
				window.document.getElementsByTagName("head")[0].appendChild(oScript);
			}
		}else {
			var aScript = ["<script src=\"", sUrl, "\" type=\"text/javascript\""];
			if(sId) {
				aScript.push([" id=\"", sId, "\""].join(""));
			}
			aScript.push("></script>");
			window.document.write(aScript.join(""));
		}
		return this;
	};
	...
	jQuery.sap.includeStyleSheet = function includeStyleSheet(sUrl, sId){
		...
		if((sId && (oOld = jQuery.sap.domById(sId)) && (bReplace = oOld.tagName === "LINK" && oOld.rel ==="stylesheet")) || window.document.body) {
			...
			var aLink = ["<link rel=\"stylesheet\" href=\"", sUrl, "\" type=\"text/css\" "];
			if(sId) {
				aLink.push([" id=\"", sId, "\""].join(""));
			}
			aLink.push(" />");
			window.document.write(aLink.join(""));
		}
		return this;
	};

	// Argh!!
	if(jQuery && jQuery().jquery.substr(0,3) === "1.3") {
		jQuery.guid = 2;
...
/**
 * Executes an 'eval' for its aguments in the global context (without closure variables)
 *
...
jQuery.sap.declare('jquery-1.4.4'); // raw module, declared by Phoenix CreateModule Ant-Task
jQuery.sap.declare('jquery-ui-position'); // raw module, declared by Phoenix CreateModule Ant-Task
jQuery.sap.declare('jquery.sap.global'); // raw module, declared by Phoenix CreateModule Ant-Task
...
// Provides the real core class sap.ui.core.Core of Phoenix
...
(function() {
	...
	jQuery.fn.cursorPos = function cursorPos(iPos) {
		...
		if( (this.length === 1 && this.attr("tagName") == "INPUT" && (this.attr("type") == "text" || this.attr("type") == "password"))
				|| this.length === 1 && this.attr("tagName") == "TEXTAREA"	) {
	 ...
	 *
	 * This method intentionally does not operate on the jQuery object, as the original jQuery.contains() method
	 * also does not do so.
	 *
	 * @param {DOMNode} oDomRefContainer The container element
	 * @param {DOMNode} oDomRefChild The child element (may not be a text node, must be an element)
	...
	jQuery.fn.rectContains = function rectContains(iPosX, iPosY) {
		jQuery.sap.assert(!isNaN(iPosX));
		jQuery.sap.assert(!isNaN(iPosY));
	 ...
	 *
	 * @return {DOMNode} the last dom ref
	 * @public
	 * @methodOf jQuery.prototype
...
(function(){
	...
	/**
	 * List of DOM events that an UIArea automatically takes care of.
	 *
	 ...
	 *
	 * Pure JavaScript listeners can evaluate the classification information with the help
	 * of the {@link jQuery.Event#isPseudoType} method.
	 *
	 * Instead of doing so, Phoenix Controls or Elements should simply implement
	 * an <code>on<i>pseudo-event</i>(oEvent)</code> method. It will be invoked only
	 * when that specific pseudo event has been recognized. This simplifies event
	...
	jQuery.sap.PseudoEvents = {
		...
		/**
		 * Pseudo event for  keyboard Home / Pos 1 with modifiers (Ctrl, Alt or Shift)
		 * @public
		 */
		...
		/**
		 * Pseudo event for  keyboard Home / Pos 1 without modifiers (Ctrl, Alt or Shift)
		 * @public
		 */
	...
	function initPseudoEventBasicTypes(){
		...
		for (var sName in mEvents) {
			if (mEvents[sName].aTypes) {
				for (j = 0, js = mEvents[sName].aTypes.length; j < js; j++) {
	 ...
	 * <b>See <a href="http://www.jquery.com">jQuery.com</a> and
	 * <a href="http://api.jquery.com/category/events/event-object/">jQuery.Event object</a>.</b>
	 *
	 * @class Placeholder for documentation of the jQuery.Event class.<br/>
	 * <b><i>See <a href="http://www.jquery.com">jQuery.com</a> and
	 * <a href="http://api.jquery.com/category/events/event-object/">jQuery.Event object</a> for details.</i></b>
	 *
	 * @name jQuery.Event
	 * @public
	 */
...
/*
 * Provides methods to create logging output for Javascript. It stores an array of log entries,
 * each containing a timestamp, level, message, details and component info.
 */
jQuery.sap.declare("jquery.sap.logger", false);

(function() {
	//ensure not to initialize twice
	if(jQuery.sap.log && jQuery.sap.log.Level) {
		return;
	}
	 ...
	 * {@link jQuery.sap.log.#trace}, {@link jQuery.sap.log.#debug}, {@link jQuery.sap.log.#info},
	 * {@link jQuery.sap.log.#warning}, {@link jQuery.sap.log.#error} and {@link jQuery.sap.log.#fatal}
	 * creates and records a log entry, containing a timestamp, a log level, a message with details and
	 * a component info. The log level will be one of {@link jQuery.sap.log.Level} and depends on
	 * the concrete logging method.
	 *
	 * By using the {@link jQuery.sap.log#setLevel} method, the user can determine the least important
	 * log level, that should be recorded. Less important entries will be filtered out.
	 * Default level is {@link jQuery.sap.log.Level.DEBUG}.
	 *
	 * {@link jQuery.sap.log#getLog} returns an array of the currently collected log entries.
	 *
	 * Last but not least, a listener can be registered to the log. It will be notified whenever a new entry
	 * is added to the log. The listener can be used for displaying log entries in a separate page are
	 ...
	 * @public
	 */
	jQuery.sap.log = /** @lends jQuery.sap.log */new (function(){
		/**
		 * Enumeration of the possible log levels for a log entry; lower values represent higher importance.
		 * The values (from most to least important) are: FATAL, ERROR, WARNING, INFO, DEBUG, TRACE.
		 *
		 * Whenever an entry is created in the log, it must be given a log level. Only if that level
		 * is not higher than the current log level {@link jQuery.sap.log.LogLevel} of this logger
		 * (which translates to 'less important'), then the entry is permanently added to the log.
		...
		this.Level = {
			...
			ERROR : 1,
			/**
			 * Warning level. Use this for logging unwanted but forseen situations
			 * @public
			 */
			WARNING : 2,
			/**
			 ...
			 * @public
			 */
			TRACE : 5 /* TODO Think about changing to 10 and thus to pull out of logging... -> Make tracing explicit */
		};
		 ...
		 * @see jQuery.sap.log#.setLevel
		 * @see jQuery.sap.log#.log
		 * @namespace
		 * @public
		 */
		this.LogLevel = {
			 ...
			 * @public
			 */
			NONE : (this.Level.FATAL - 1),
			/**
			 * Log events with Level &lt;= {@link jQuery.sap.log.Level.FATAL}
			 * @public
			 */
			FATAL : this.Level.FATAL,
			/**
			 * Log events with Level &lt;= {@link jQuery.sap.log.Level.ERROR}
			 * @public
			 */
			ERROR : this.Level.ERROR,
			/**
			 * Log events with Level &lt;= {@link jQuery.sap.log.Level.WARNING}
			 * @public
			 */
			WARNING : this.Level.WARNING,
			/**
			 * Log events with Level &lt;= {@link jQuery.sap.log.Level.INFO}
			 * @public
			 */
			INFO  : this.Level.INFO,
			/**
			 * Log events with Level &lt;= {@link jQuery.sap.log.Level.DEBUG}
			 * @public
			 */
			DEBUG : this.Level.DEBUG,
			/**
			 * Log events with Level &lt;= {@link jQuery.sap.log.Level.TRACE}
			 * @public
			 */
			TRACE : this.Level.TRACE, /* TODO Think about changing to 10 and thus to pull out of logging... -> Make tracing explicit */
			/**
			 * Log all events
			 * @public
			 */
			ALL : (this.Level.TRACE + 1) /* TODO if TRACE is changed to make sure this is 6 again. There would then be some special TRACE handling. */
		};

		/**
		 * Map holding some mapping information that can be used for getting the index of a LogLevel
		 * @private
		 */
		var mBackMapping = [];
		jQuery.each(this.LogLevel, function(idx, v){
			mBackMapping[v] = idx;
		});
		...
		/**
		 * Maximum log level to be recorded.
		 * @private
		 */
		var iMaxLevel = this.LogLevel.DEBUG;
		...
		/**
		 * Creates a new entry in the log with the given level, message, details and reporting component.
		 *
		 * @param {jQuery.sap.log.Level} iLevel the level of the log entry
		...
		function log(iLevel, sMessage, sDetails, sComponent) {
			if (iLevel <= iMaxLevel) {
				var oNow = new Date(),
					sDate = formatDate(oNow),
					sTime = formatTime(oNow),
					oLogEntry = {
						date     : sDate,
						time     : sTime,
						timestamp: sDate + ' ' + sTime,
						level    : iLevel,
						message  : sMessage ? sMessage : "No Message",
						details  : sDetails ? sDetails : "",
						component: sComponent ? sComponent : ""
					};
				aLog.push( oLogEntry );
				if (oListener) {
				...
				// Also try to log to the IE8, Firebug or Companion.js console, if available. Usage is restricted to a common console API subset.
				if (window.console) {
					var logText = oLogEntry.timestamp + " " + oLogEntry.message + " - " + oLogEntry.details + " " + oLogEntry.component;
					if ((iLevel <= jQuery.sap.log.Level.ERROR) && console.error) { // ERROR and FATAL
						console.error(logText);
					} else if ((iLevel <= jQuery.sap.log.Level.WARNING) && console.warn) { // WARNING
						console.warn(logText);
					} else if ((iLevel === jQuery.sap.log.Level.DEBUG) && console.debug) { // DEBUG
						console.debug(logText);
					} else if ((iLevel === jQuery.sap.log.Level.DEBUG) && console.log) { // "DEBUG" for IE
						console.log(logText);
					} else if ((iLevel <= jQuery.sap.log.Level.DEBUG) && console.info) { // INFO (and DEBUG)
						console.info(logText);
					} else if ((iLevel <= jQuery.sap.log.Level.TRACE) && console.trace) { // trace
		 ...
		 * @param {string} [sDetails=""] details about the message, might be omitted
		 * @param {string} [sComponent=""] name of the component that produced the log entry
		 * @return {jQuery.sap.log} the log-instance
		 * @public
		 */
		this.fatal = function fatal(sMessage, sDetails, sComponent) {
			log(jQuery.sap.log.Level.FATAL, sMessage, sDetails, sComponent);
			return this;
		};
		/**
		...
		this.error = function error(sMessage, sDetails, sComponent) {
			log(jQuery.sap.log.Level.ERROR, sMessage, sDetails, sComponent);
			return this;
		};
		/**
		...
		this.warning= function warning(sMessage, sDetails, sComponent) {
			log(jQuery.sap.log.Level.WARNING, sMessage, sDetails, sComponent);
			return this;
		};
		/**
		...
		this.info = function info(sMessage, sDetails, sComponent) {
			log(jQuery.sap.log.Level.INFO, sMessage, sDetails, sComponent);
			return this;
		};
		/**
		...
		this.debug = function debug(sMessage, sDetails, sComponent) {
			log(jQuery.sap.log.Level.DEBUG, sMessage, sDetails, sComponent);
			return this;
		};
		/**
		...
		this.trace = function trace(sMessage, sDetails, sComponent) {
			log(jQuery.sap.log.Level.TRACE, sMessage, sDetails, sComponent);
			return this;
		};

		/**
		 * Returns the logged entries recorded so far as an array
		 * @return {array} an array containing the logged entries recorded so far
		 * @public
		 */
		this.getLog = function getLog() {
			// TODO should this really return the original array or clone it via aLog.slice()?
			return aLog;
		};

		/**
		 * Allows setting the current jQuery.sap.log.LogLevel
		 * which determines the level of log entries to be determined.
		 * @param {jQuery.sap.log.LogLevel} iLogLevel
		 * @return {jQuery.sap.log} the log-instance
		 * @public
		 */
		this.setLevel = function setLevel(iLogLevel) {
			iMaxLevel = iLogLevel;
			log(iLogLevel, "Changing level to " + mBackMapping[iLogLevel], "", "jQuery.sap.log");
			return this;
		};
		 ...
		 * @public
		 */
		this.setLogListener = function setLogListener(oNewListener) {
		 ...
		 * @param {Date} oDate the date that should be formatted
		 * @returns {String} date information of the given oDate as formatted string, including milliseconds in case of bMillis was <code>true</code>
		 * @private
		 */
		 ...
		 * @param {Date} oDate the date that should be formatted
		 * @param {boolean} [bMillis=false] whether milliseconds should be included in the formatted output.
		 * @returns {String} time information of the given oDate as formatted string, including milliseconds in case of bMillis was <code>true</code>
		 * @private
		 */
		function formatTime(oDate, bMillis) {
			var sHours = oDate.getHours()<10?"0"+oDate.getHours():""+oDate.getHours(),
				...
				sMillis = oDate.getMilliseconds()<10?"00"+oDate.getMilliseconds():oDate.getMilliseconds()<100?"0"+oDate.getMilliseconds():""+oDate.getMilliseconds();
			if ( bMillis !== false ) {
	...
	// end of log instance creation and registration

// end of anonymous function creating the Logger instance and the global log method implementations
}());
...
(function() {
	...
	// Nice Javadoc for private inner class "Properties" - this list of comments is intentional!
	/**
	 ...
	 *
	 * The getProperty methods is used to retrieve a value from the list.
	 * The setProperty method is used to store or change a property in the list.
	 * Additionally one can use the getKeys method to retrieve an array of all keys that are
	 ...
	 */
	/**
	 * Returns the value of a given key. Optional a given default value is returned if the requested key is not in the list.
	...
	function parse(sText, oProp) {
		...
		for (var i = 0; i < aLines.length; i++) {
			...
			while (sLine.lastIndexOf("\\") == sLine.length-2) {
...
(function() {
	...
	// Nice Javadoc for private inner class "Bundle" - this list of comments is intentional!
	/**
	 * @interface  A Resource Bundle contains locale-specific texts.
	 *
	 * If you need a locale specific text within you application you can use the
	 ...
	 *
	 * You have to specify there the URL to the base .properties file of a bundle
	 ...
	 *
	 * With the given locale the ResourceBundle requests the locale-specific properies file
	 * (e.g. "mybundle_fr_FR.properties").
	 * If no file is found for the locale "fr_FR" the fallback is requested.
	 * The fallback is the language code without a specific country code in this case "fr".
	 * Same counts if the getText method did not find a string value for a given key.
	 * If no language specific file is found the English file is requested with locale "en.
	 ...
	 * @param {string} sKey
	 * @param {string[]} aArgs List of strings which should replace the placeholders "{x}" (x is the index) in the found locale-specific string value.
...
(function() {
	...
	// Nice Javadoc for private inner class "UriParams" - this list of comments is intentional!
	/**
	...
	var UriParams = function() {
		this.mParams = {};
		var sQueryString = window.document.location.href;
		if(sQueryString.indexOf("?") > 0){
			sQueryString = sQueryString.substring(sQueryString.indexOf("?") + 1);
			...
			for(var i=0; i<aParameters.length; i++){
				...
				if(sName){
					if(!jQuery.isArray(mParameters[sName])){
						mParameters[sName] = [];
					}
	...
	UriParams.prototype.get = function(sName, bAll) {
		var bReturnFirstOnly = true;
		if(bAll && typeof(bAll) == "boolean"){
			bReturnFirstOnly = false;
		}
		var aParamValues = this.mParams[sName];
		var oResult = bReturnFirstOnly ? null : [];
		if(jQuery.isArray(aParamValues)){
			if(bReturnFirstOnly){
				if(aParamValues.length > 0){
					oResult = aParamValues[0];
				}
			}else{
				oResult = aParamValues;
			}
		}
		return oResult;
	};
	 ...
	 *
	 * <b>Example for reading a single Uri parameter (or the value of the first
	 * occurrence of the Uri parameter:</b>
	 * <pre>
	 *  var sValue = jQuery.sap.getUriParameters().get("myUriParam");
	 * </pre>
	 *
	 * <b>Example for reading the values of the first of the Uri parameter
	 * (with multiple occurrences):</b>
	 * <pre>
	 ...
	 */
	jQuery.sap.getUriParameters = function getUriParameters() {
		return new UriParams();
	};

	/**
	 * Sorts the given array inplace and removes any duplicates (identified by "===").
	 *
	 * Use <code>jQuery.unique()</code> for arrays of DOMElements.
	 *
	 * @param [Array} a Array of any type
	 * @return {Array} same array as given (for chaining)
	 * @public
	 */
	jQuery.sap.unique = function(a) {
		jQuery.sap.assert(a instanceof Array);
		var l=a.length;
		if ( l > 1 ) {
			a.sort();
...
sap.ui = jQuery.extend(sap.ui, {
		 ...
		 */
		version: "0.18.0-SNAPSHOT",
		buildinfo : { lastchange : "unknown", buildtime : "201108190214" }
	});
 ...
 *
 * The namespace is given as a dot separated list of namespaces (<code>sNamespace</code>).
 * If any namespace in the hierarchy does not exist yet, it is created as an empty object
 * (equivalent to <code>var namespace = {};</code>). The inner most namespace object is
 * returned as result of this function.
 *
 * By convention, the name of each namespace should consist of lowercase ASCII characters,
 * digits and underscores only and it should start with a letter. So the name should follow
 * the regular expression </code>[a-z][a-z0-9_]*</code>.
 *
 * <b>Example:</b>
 * <pre>
 *   sap.ui.namespace("sample.uilib");
 * </pre>
 * is equivalent to
 * <pre>
 *   if ( !window["sample"] ) {
 *     window["sample"] = {};
 *   }
 *   if ( !window["sample"]["uilib"] ) {
 *     window["sample"]["uilib"] = {};
 *   }
 *   return window["sample"]["uilib"];
 * </pre>
 *
 * While concept and structure of the namespaces follow the suggestions of the OpenAjax
 * initiative, any namespace object created by this method <b>is not automatically
 * registered with the OpenAjax hub</b>. If such a registration is desired, callers
 * have to do it on their own.
 *
 * <b>Note</b>: The namespace "sap.ui.global" is reserved and its usage is forbidden.
 *
 * @see http://www.openajax.org
 *
 * @param {string} sNamespace Name of the package to register
 * @type {object}
...
/**
 * Prepares a class with a given <code>sClassName</code> for later usage.
 *
 * If the class has been loaded already, nothing is done. Otherwise a stub
 * for the constructor of the class is created which loads the class on demand,
 * replaces itself with the real constructor and then delegates to it.
 *
 * As the stub mechanism relies on the constructor function, this method <b>must
 * not be called for static classes (namespaces)</b> which don't have constructors.
 *
 * The class name has to be specified in the same notation as documented
 * for {@link #require}.
 *
 * <b>Note</b>: As a side effect of this method, the namespace that will contain the
 * class is <b>immediately</b> created.
 *
 * @param {String}  sClassName Fully qualified name of the class that should be prepared
 * @public
 * @static
 * @no-rename
 */
sap.ui.lazyRequire = function(sClassName, sMethods) {

	jQuery.sap.assert(typeof sClassName === "string");
	jQuery.sap.assert(!sMethods || typeof sMethods === "string");

	var bConstructor = typeof sMethods !== "string",
		sFullClass = sClassName.replace(/\//gi,"\."),
	...
	// Check whether class is already available
	if (oClass) {
		return;
	}
	...
	if ( bConstructor ) {
		...
		oPackage[sClass] = function() {
			var oRealClass = loadClass();
			jQuery.sap.assert(typeof oRealClass == "function");
		...
		// mark the function as lazy loader so that require() can properly do its job
		oPackage[sClass]._isLazyLoader = true;
		...
		jQuery.each(sMethods.split(" "), function (i,sMethod) {
			// check whether method is already available
			if (!oPackage[sClass][sMethod]) {
				oPackage[sClass][sMethod] = function() {
					jQuery.sap.log.debug("lazy stub for '" + sFullClass + "." + sMethod + "' called.");
					var oRealClass = loadClass();
					jQuery.sap.assert(typeof oRealClass == "function" || typeof oRealClass == "object");
					jQuery.sap.assert(typeof oRealClass[sMethod] == "function");
	...
	function loadClass() {
		...
		var oClass = oPackage[sClass];

		// load and execute module
		jQuery.sap.require(sFullClass);

		// get the real class
		var oRealClass = oPackage[sClass];

		// oRealClass must not point to the dummy constructor anymore
		jQuery.sap.assert(oRealClass != oClass);
...
sap.ui.base.Metadata = function(sClassName, oStaticInfo) {
	...
	jQuery.sap.assert(!!sClassName && typeof sClassName == "string");
	jQuery.sap.assert(!oStaticInfo || typeof oStaticInfo == "object");
	...
	if ( oStaticInfo.baseType ) {
		...
		if ( oParentClass.getMetadata ) {
			...
			jQuery.sap.assert(oParentClass === oParentClass.getMetadata().getClass());
		} else {
 ...
 *
 * @class Base class for all Phoenix Objects
 * @abstract
 * @author Malte Wedel
 * @version 0.18.0-SNAPSHOT
 * @public
...
sap.ui.base.Event.prototype.init = function(sId, oSource, mParameters) {
	jQuery.sap.assert(typeof(sId) == "string");
	jQuery.sap.assert(oSource instanceof sap.ui.base.EventProvider);

	this.sId = sId;
	this.oSource = oSource;
	this.mParameters = mParameters ? mParameters : {};
};
...
sap.ui.base.Event.prototype.getParameter = function(sName) {
	...
	jQuery.sap.assert(typeof(sName) == "string");
	jQuery.sap.assert(sName != "");
 ...
 * @public
 */
sap.ui.base.EventProvider.prototype.attachEvent = function(sEventId, oData, fFunction, oListener) {
	jQuery.sap.assert(typeof(sEventId) === "string" && sEventId );
	...
	jQuery.sap.assert(typeof(fFunction) === "function");
	jQuery.sap.assert(!oListener || typeof(oListener) === "object");

	if (!this.mEventRegistry[sEventId]) {
		this.mEventRegistry[sEventId] = [];
	}
	this.mEventRegistry[sEventId].push({oListener:oListener, fFunction:fFunction, oData: oData});
 ...
 * @public
 */
sap.ui.base.EventProvider.prototype.detachEvent = function(sEventId, fFunction, oListener) {
	...
	for (var i = 0, iL = aEventListeners.length; i < iL; i++) {
		...
		if (aEventListeners[i].fFunction === fFunction && aEventListeners[i].oListener === oListener) {
 ...
 * @protected
 */
sap.ui.base.EventProvider.prototype.fireEvent = function(sEventId, mParameters) {
...
//	jQuery.sap.assert(typeof (sEventId) == "string");
	...
	// this ensures no 'concurrent modification exception' occurs (e.g. an eventlisteners deregisters itself).
	aEventListeners = aEventListeners.slice();

	var oEvent = this.oEventPool.borrowObject(sEventId, this, mParameters);

	//PERFOPT use array. remember length to not re-calculate over and over again
	for (var i = 0, iL = aEventListeners.length; i < iL; i++) {
		var oListenerInfo = aEventListeners[i],
			fFunction = oListenerInfo.fFunction,
			oListener = oListenerInfo.oListener,
			oData = oListenerInfo.oData;
		if (oListener) {
			// try {
			fFunction.call(oListener, oEvent, oData);
			// } catch(oException){
			// jQuery.sap.log.fatal("EventHandler not existing on Listener object.");
			// TODO How do we actually figure out that calling this very method on this very Listener Object did fail and not
			// something in the event handling?
			// jQuery.sap.log.error("Exception during fireEvent-method.", oException.message,
			// (this.getMetadata?this.getMetadata().getName():'EventProvider'));
			// throw oException;
			// }
		} else {
			fFunction(oEvent, oData);
		}
	}
...
sap.ui.core.Configuration = function() {
	...
	var M_SETTINGS = {
		...
		// "timezone"      : "UTC",
		"accessibility"   : { type : "boolean",  defaultValue : false },
	...
	// 2.-4. apply settings from global config object (already merged with script tag attributes)
	...
	// 5. apply the settings from the url (only if not blocked by app configuration)
...
sap.ui.base.Object.defineClass("sap.ui.core.Configuration", {
	baseType : "sap.ui.base.Object",
	publicMethods : ["getTheme", /*"setTheme", */"getLanguage", "getAccessibility", "getAnimation", "getRTL", "getDebug", "getNoDuplicateIds", "getUIDPrefix"]
});
...
sap.ui.base.DataType.prototype.parseValue = function(sValue) {
	// currently this function considers to handle primitive values and
	// primitive arrays - in future may be other values might be also relevant.
	...
	if (sType == "string") {
		...
		for (var i = 0; i < aValues.length; i++) {
			aValues[i] = aValues[i] == "true";
		}
		return aValues;
	} else if (sType == "int[]") {
		var aValues = sValue.split(",");
		for (var i = 0; i < aValues.length; i++) {
			aValues[i] = parseInt(aValues[i]);
		}
		return aValues;
	} else if (sType == "float[]") {
		var aValues = sValue.split(",");
		for (var i = 0; i < aValues.length; i++) {
			aValues[i] = parseFloat(aValues[i]);
		}
		return aValues;
	} else {
...
(function() {
	...
	function createType(name, s, base) {
		...
		jQuery.sap.assert(typeof name === "string" && !!name);
		jQuery.sap.assert(!base || base instanceof sap.ui.base.DataType);
		s = s || {};
	...
	var getArrayValidator = function(fIsValid) {
		return function(vValue) {
			if (vValue === null) {
				return true;
			}
			if (typeof vValue === "object" && vValue instanceof Array) {
				for (var i = 0; i < vValue.length; i++) {
					if (!fIsValid(vValue[i])) {
						return false;
					}
				}
				return true;
	...
	var PREDEFINED_TYPES = {
		...
		"boolean[]" :
			createType("boolean[]", {
				defaultValue : null,
				isValid : getArrayValidator(function(vValue) {
					return typeof vValue === "boolean";
				})
			}),
...
(function() {
	...
	sap.ui.core.ElementMetadata.prototype.uid = function() {
		...
		if ( typeof sId !== "string" ) {
			...
			sId = sId.replace(/([^A-Za-z0-9-_.:])|([0-9]$)/g,"").toLowerCase();
			// add configured prefix and remember token
			sId = this._sUIDToken = sap.ui.getCore().getConfiguration().getUIDPrefix() + sId;
			// ensure a valid UIDCount
			mUIDCounts[sId] = mUIDCounts[sId] || 0;
		}

		// concatenating sId and a counter is only safe because we remove trailing numbers in sId!
...
// chain the prototypes
sap.ui.model.Model.prototype = jQuery.sap.newObject(sap.ui.base.Object.prototype);
 ...
 * @param {object}
 *         oContext the context object for this databinding (optional)
 ...
 *         oSorter initial sort order (optional)
 * @param {array}
 *         aFilters predefined filter (optional)
 ...
 * @param {object}
 * 		   oContext the context which should be used to create the new binding context
 * @param {function}
 ...
 * @param {object}
 * 		   oContext the context which the path should be resolved
 * @public
 */
...
sap.ui.core.Element.defineClass("sap.ui.core.Element", {
  ...
  publicMethods : [ "getId", "getMetadata", "getTooltip", "setTooltip", "getTooltip_AsString", "getTooltip_Text", "getModel", "setModel", "bindProperty", "unbindProperty", "bindAggregation", "unbindAggregation" ],
  library : "sap.ui.core",
  properties : {
...
sap.ui.core.Element.prototype._handleEvent = function (oEvent) {
	...
	if (this.aDelegates.length > 0) {
		for (var i = 0; i < this.aDelegates.length; i++) {
			var oDelegate = this.aDelegates[i];
			if (oDelegate[sHandlerName]) {
				oDelegate[sHandlerName](oEvent);
		   }
		}
	}
};
		...
		if ( oKeyInfo = mValidKeys[sKey] ) {
			...
			case 0: // PROPERTY
				if (this.isBinding(oValue, oKeyInfo)) {
					...
					this.bindProperty(sKey, oBindingInfo.path, oBindingInfo.formatter);
				} else {
 ...
 * @return {sap.ui.core.Element} Returns <code>this</code> to allow method chaining
 * TODO better name bSuppressRerendering positive, e.g. "bStayValid"
 * @protected
 */
sap.ui.core.Element.prototype.setProperty = function(sPropertyName, oValue, bSuppressRerendering) {
 ...
 * @param {boolean}
 *            [bSuppressRerendering] if true, the UI element is not marked for redraw
 * @return {sap.ui.core.Element} Returns <code>this</code> to allow method chaining
 * @protected
 */
sap.ui.core.Element.prototype.setAssociation = function(sAssociationName, sId, bSuppressRerendering) {
...
sap.ui.core.Element.prototype.getAssociation = function(sAssociationName, oDefaultForCreation) {
	...
	if (!result) {
		result = this.mAssociations[sAssociationName] = oDefaultForCreation ? oDefaultForCreation : null;
	} else {
 ...
 * @param {boolean}
 *            [bSuppressRerendering] if true, the UI element as well as the newly associated element is not marked for redraw
 * @return {sap.ui.core.Element} Returns <code>this</code> to allow method chaining
 * @protected
 */
sap.ui.core.Element.prototype.addAssociation = function(sAssociationName, sId, bSuppressRerendering) {
	if (!sId) {
		return this;
	}
 ...
 * @protected
 */
sap.ui.core.Element.prototype.removeAssociation = function(sAssociationName, vElement, bSuppressRerendering) {
	var aIds = this.mAssociations[sAssociationName];
	var sId = null;
 ...
 * @protected
 */
sap.ui.core.Element.prototype.removeAllAssociation = function(sAssociationName, bSuppressRerendering){
	var aIds = this.mAssociations[sAssociationName];
	if (!aIds)	{
		return [];
	}
 ...
 * @protected
 */
sap.ui.core.Element.prototype.setAggregation = function(sAggregationName, oElement, bSuppressRerendering) {
	...
	if (oElement instanceof sap.ui.core.Element) { // adopt new child
		oElement.setParent(this, sAggregationName, bSuppressRerendering);
	} else {
		if (!bSuppressRerendering) {
			this.invalidate();
		}
...
sap.ui.core.Element.prototype.getAggregation = function(sAggregationName, oDefaultForCreation) {
	...
	if (!aChildren) {
		aChildren = this.mAggregations[sAggregationName] = oDefaultForCreation
		? oDefaultForCreation
		: null;
	}
	if (aChildren) {
 ...
 *            the last position
 * @param {boolean}
 *            [bSuppressRerendering] if true, the UI element as well as the added child is not marked for redraw
 * @return {sap.ui.core.Element} Returns <code>this</code> to allow method chaining
 * @protected
 */
sap.ui.core.Element.prototype.insertAggregation = function(sAggregationName, oElement, iIndex, bSuppressRerendering) {
	if (!oElement) {
		return this;
	}
 ...
 * @protected
 */
sap.ui.core.Element.prototype.addAggregation = function(sAggregationName, oElement, bSuppressRerendering) {
	if (!oElement) {
		return this;
	}
 ...
 * @protected
 */
sap.ui.core.Element.prototype.removeAggregation = function(sAggregationName, vElement, bSuppressRerendering) {
	var aChildren = this.mAggregations[sAggregationName];
 ...
 * @protected
 */
sap.ui.core.Element.prototype.removeAllAggregation = function(sAggregationName, bSuppressRerendering){
	var aChildren = this.mAggregations[sAggregationName];
	if (!aChildren)	{
		return [];
	}
 ...
 * @protected
 */
sap.ui.core.Element.prototype.destroyAggregation = function(sAggregationName, bSuppressRerendering){
	var aChildren = this.mAggregations[sAggregationName];
	if (!aChildren) {
		return this;
	}
	...
	if (aChildren instanceof sap.ui.core.Element) {
		aChildren.destroy(bSuppressRerendering);
	} else if (aChildren && jQuery.isArray(aChildren)) {
		for (var i = 0; i < aChildren.length; i++) {
			if (aChildren[i]) {
				aChildren[i].destroy(bSuppressRerendering);
			}
		}
	}
 ...
 * @protected
 */
sap.ui.core.Element.prototype.invalidate = function(oOrigin) {
	var oParent = this.getParent();
	if (oParent) {
 ...
 * @protected
 */
sap.ui.core.Element.prototype._removeChild = function(oChild, sAggregationName, bSuppressRerendering) {
	if (!sAggregationName) {
		...
		if (iIndex == -2) { // 0..1
			if(this[oAggregationInfo._sMutator]) {
				...
				this.setAggregation(sAggregationName, null, bSuppressRerendering);
			}
			...
			if(this[oAggregationInfo._sRemoveMutator]) {
				...
				this.removeAggregation(sAggregationName, iIndex, bSuppressRerendering);
			}
		} else {
			// already removed!?
 ...
 * @param {boolean}
 *            [bSuppressRerendering] if true, the UI element is not marked for redraw. The old parent, however, is marked for redraw.
 * @return {sap.ui.core.Element}
 *            Returns <code>this</code> to allow method chaining
 * @private
 */
sap.ui.core.Element.prototype.setParent = function(oParent, sAggregationName, bSuppressRerendering) {
	if ( !oParent ) {
		this.oParent = null;
	...
	if((!this.bPrerendered || this.bPrerendered && oOldParent) && !bSuppressRerendering) {
		this.invalidate();
	}
 ...
 * @public
 */
sap.ui.core.Element.prototype.destroy = function(bSuppressRerendering) {
	var that = this;
	...
	for(var oAggr in this.mAggregations){
		this.destroyAggregation(oAggr, bSuppressRerendering);
	}
	this.deregister();

	// remove this child from parent aggregation
	if (this.getParent() && this.sParentAggregationName) {
		this.getParent()._removeChild(this, this.sParentAggregationName, bSuppressRerendering);
	}
...
sap.ui.core.Element.prototype.fireEvent = function(sEventId, mParameters) {
	...
	sap.ui.base.EventProvider.prototype.fireEvent.apply(this,arguments);
	if (bProcess) {
 ...
 * @private
 */
sap.ui.core.Element.prototype.addDelegate = function (oDelegate) {
 ...
 * @param {string} sPath the binding path
 * @param {function} fnFormatter the formatter function
 *
 * @return {sap.ui.core.Element} reference to the instance itself
 * @public
 */
sap.ui.core.Element.prototype.bindProperty = function(sName, sPath, fnFormatter) {
	var oBindingInfo = {path: sPath, formatter: fnFormatter};
...
sap.ui.core.Element.prototype._bindProperty = function(sName, oBindingInfo) {
	var oBinding = this.getModel().bindProperty(oBindingInfo.path, this.getBindingContext()),
 ...
 * @private
 */
sap.ui.core.Element.prototype.updateBindings = function(bRecreate) {
	var that = this;

	// create property and aggregation bindings if they don't exist yet
	jQuery.each(this.mBindingInfos, function(sName, oBindingInfo) {
		if ( bRecreate && oBindingInfo.binding ) {
			oBindingInfo.binding.detachChange(oBindingInfo.handler);
	...
	jQuery.each(this.mAggregations, function(sName, oAggregation) {
		if (oAggregation instanceof sap.ui.core.Element) {
			oAggregation.updateBindings(bRecreate);
		} else if (oAggregation instanceof Array) {
			for (var i = 0; i < oAggregation.length; i++) {
				oAggregation[i].updateBindings(bRecreate);
			}
		}
	});
...
sap.ui.core.Element.prototype.unbindProperty = function(sName){
	var oPropertyBinding = this.mBindingInfos[sName];
	if(oPropertyBinding) {
		if (oPropertyBinding.binding) {
			oPropertyBinding.binding.detachChange(oPropertyBinding.handler);
		}
...
sap.ui.core.Element.prototype.unbindAggregation = function(sName){
	var oAggregationBinding = this.mBindingInfos[sName];
	if(oAggregationBinding) {
		if (oAggregationBinding.binding) {
			oAggregationBinding.binding.detachChange(oAggregationBinding.handler);
		}
		delete this.mBindingInfos[sName];
	}
	return this;
 ...
 * @private
 */
sap.ui.core.Element.prototype.setBindingContext = function(oContext, bSkipLocal){
	this.oBindingContext = oContext;
	this.updateBindingContext(bSkipLocal);
};
	...
	if (this.sBindingPath && !bSkipLocal) {
		...
		if (oParent && oModel) {
			oModel.createBindingContext(this.sBindingPath, oParent.getBindingContext(), function(oContext) {
				that.setBindingContext(oContext, true);
			});
		}
		return;
	}
 ...
 * @public
 */
sap.ui.core.Element.prototype.setModel = function(oModel) {
	if ( oModel !== this.oModel ) {
		...
		this.updateBindings(/* bRecreate */ true);
	}
	return this;
};
 ...
 * @public
 */
sap.ui.core.Element.prototype.getModel = function() {
	if (this.oModel) {
		return this.oModel;
	}
	return this.getParent() && this.getParent().getModel();
};
...
sap.ui.core.Element.prototype.clone = function(sIdSuffix){
	var oClass = this.getMetadata()._oClass,
		sId = (this.getMetadata().uid() || jQuery.sap.uid()) + "-" + sIdSuffix,
	...
	jQuery.each(this.mBindingInfos, function(sName, oBindingInfo) {
		if (oBindingInfo.template) {
			oClone.bindAggregation(sName, oBindingInfo.path, oBindingInfo.template);
		} else {
			oClone.bindProperty(sName, oBindingInfo.path, oBindingInfo.formatter);
		}
	});
...
sap.ui.core.Element.defineClass("sap.ui.core.Control", {
	...
	publicMethods: ["addStyleClass", "removeStyleClass", "hasStyleClass", "placeAt", "attachBrowserEvent", "detachBrowserEvent"],
	library: "sap.ui.core",
	properties : {},
	aggregations : {},
	associations : {},
...
sap.ui.core.Control.prototype.clone = function() {
	...
	if ( this.aBindParameters ) {
		for(var i=0, l=this.aBindParameters.length; i<l; i++) {
			oClone.attachBrowserEvent(this.aBindParameters[i].sEventType, this.aBindParameters[i].fHandler);
		}
	}
...
sap.ui.core.Control.prototype.invalidate = function(oOrigin) {
	...
	if ( this.bOutput && (oUIArea = this.getUIArea()) ) {
		...
		if (oParent && (this.bOutput || !(this.getVisible && this.getVisible() === false))) {
			// if this control has not yet produced output, but is
			// willing to do so, or if it became invisible,
...
/**
 * Any (non-null) string given here will be added to the "class" attribute of this control.
 ...
 *
 * This method is intended to be used to mark some controls as being of a special type for which
 * special styling can be provided using CSS selectors that reference this style class name.
 *
 * @param {string} [sStyleClass] the style to be added
 * @return {sap.ui.core.Control} Returns <code>this</code> to allow method chaining
 * @public
 */
sap.ui.core.Control.prototype.addStyleClass = function(sStyleClass, bSuppressRerendering) { // bSuppressRerendering is experimental and hence undocumented
	if (!this.aCustomStyleClasses) {
 ...
 * @public
 */
sap.ui.core.Control.prototype.removeStyleClass = function(sStyleClass, bSuppressRerendering) { // bSuppressRerendering is experimental and hence undocumented
 ...
 * @public
 */
sap.ui.core.Control.prototype.hasStyleClass = function(sStyleClass) {
	if (sStyleClass && this.aCustomStyleClasses) {
		for (var i = this.aCustomStyleClasses.length-1; i >= 0; i--) {
			if (this.aCustomStyleClasses[i] == sStyleClass) {
				return true;
 ...
 * Phoenix control events, as e.g. accessibility-related functionality is then provided automatically.
 ...
 *
 * In the event handler, "this" refers to the Control's root DOM element. While it can be used and modified, the general
 * caveats for working with Phoenix control DOM elements apply. In particular the DOM element may be destroyed and replaced by a new
 * one at any time, so modifications that are required to have permanent effect may not be done. E.g. use Control.addStyleClass()
 ...
 * @public
 */
sap.ui.core.Control.prototype.attachBrowserEvent = function(sEventType, fHandler) {
	if (sEventType && (typeof(sEventType) == "string")) { // do nothing if the first parameter is empty or not a string
		if (fHandler && typeof(fHandler) == "function") {   // also do nothing if the second parameter is not a function
			// store the parameters for bind()
			if (!this.aBindParameters) {
				this.aBindParameters = [];
			}
			this.aBindParameters.push({"sEventType":sEventType, "fHandler": fHandler});

			// if control is rendered, directly call bind()
			var oDomRef = this.getDomRef();
			if (oDomRef) {
				jQuery(oDomRef).bind(sEventType, fHandler);
			}
		}
	}
 ...
 *
 * @param {string} [sEventType] A string containing one or more JavaScript event types, such as "click" or "blur".
 * @param {function} [fHandler] The function that is to be no longer executed.
 * @public
 */
sap.ui.core.Control.prototype.detachBrowserEvent = function(sEventType, fHandler) {
	if (sEventType && (typeof(sEventType) == "string")) { // do nothing if the first parameter is empty or not a string
		if (fHandler && typeof(fHandler) == "function") {   // also do nothing if the second parameter is not a function

			// remove the bind parameters from the stored array
			if (this.aBindParameters) {
				for (var i = this.aBindParameters.length - 1; i >= 0; i--) {
					var oParamSet = this.aBindParameters[i];
					if ((oParamSet.sEventType == sEventType) && (oParamSet.fHandler === fHandler)) {
			...
			var oDomRef = this.getDomRef();
			if (oDomRef) {
				jQuery(oDomRef).unbind(sEventType, fHandler);
			}
		}
	}
...
sap.ui.core.Control.prototype.placeAt = function(oRef, oPosition) {
	...
	if(oCore.isInitialized()){
		...
		if(!bIsUIArea){
			...
			if(!oContentAggInfo || !oContentAggInfo.multiple || oContentAggInfo.type != "sap.ui.core.Control"){
...
(function(){
	...
	sap.ui.core.FocusHandler = function(oRootRef, oCore) {
		...
		if(oRootRef.addEventListener){ //FF, Safari, IE>=9
			oRootRef.addEventListener(jQuery.browser.msie ? "focusin" : "focus", this.fEventHandler, true);
			oRootRef.addEventListener(jQuery.browser.msie ? "focusout" : "blur", this.fEventHandler, true);
		}else{ //IE8
	...
	sap.ui.core.FocusHandler.prototype.destroy = function(event) {
		...
		if(oRootRef){
			if(oRootRef.removeEventListener){ //FF, Safari, IE>=9
				oRootRef.removeEventListener(jQuery.browser.msie ? "focusin" : "focus", this.fEventHandler, true);
				oRootRef.removeEventListener(jQuery.browser.msie ? "focusout" : "blur", this.fEventHandler, true);
			}else{ //IE 8
...
(function(){
	...
	sap.ui.core.ResizeHandler.prototype.attachListener = function(oDomRef, fHandler){
		...
		if(jQuery.browser.msie && (jQuery.browser.version == "7.0" || jQuery.browser.version == "8.0")){
			...
			if(!this.sInterval){
				this.sInterval = jQuery.sap.intervalCall(this.I_INTERVAL, this, "checkSizes");
			}
		}
		return sId;
	};
	...
	sap.ui.core.ResizeHandler.prototype.detachListener = function(sId){
		...
		if(this.aResizeListeners.length == 0){
			jQuery.sap.clearIntervalCall(this.sInterval);
			this.sInterval = null;
		}
	};
...
(function() {
	...
	var aCommonMethods = ["renderControl", "write", "writeEscaped", "translate", "writeAcceleratorKey", "writeControlData",
						  ...
						  "writeCustomStyleClasses", "addStyle", "writeStyles", "getEscaped", "writeAccessibilityState",
						  "getConfiguration", "getResourceRoot", "getHTML"];
 ...
 * Shortcut for <code>jQuery.sap.getModulePath('', '/')</code>
 * @return {string} the root path of the resources
 * @deprecated Since version 0.15.0. Use the more flexible {@link jQuery.sap.getModulePath} instead.
 * @public
 */
sap.ui.core.RenderManager.prototype.getResourceRoot = function() {
	return jQuery.sap.getModulePath('', '/');
};
...
sap.ui.core.RenderManager.prototype.renderControl = function(oControl) {
	...
	if (oControl.aBindParameters && oControl.aBindParameters.length > 0) { // if we have stored bind calls...
		var jDomRef = jQuery(oControl.getDomRef());
		if (jDomRef && jDomRef[0]) { // ...and we have a DomRef
			for (var i = 0; i < oControl.aBindParameters.length; i++) {
				var oParams = oControl.aBindParameters[i];
				jDomRef.unbind(oParams.sEventType, oParams.fHandler);
			}
		}
	}
...
(function() {
		...
		for (var i = 0, size = aRenderedControls.length; i < size; i++) {
			...
			if (oControl.aBindParameters && oControl.aBindParameters.length > 0) { // if we have stored bind calls...
				...
				if (jDomRef && jDomRef[0]) { // ...and we have a DomRef - TODO: this check should not be required right after rendering...
					for (var j = 0; j < oControl.aBindParameters.length; j++) {
						var oParams = oControl.aBindParameters[j];
						jDomRef.bind(oParams.sEventType, oParams.fHandler);
					}
				}
			}
		}
	 ...
	 * @public
	 */
	sap.ui.core.RenderManager.prototype.flush = function(oTargetDomNode, bDoNotPreserve) {
		...
		if(jQuery.browser.safari){
			// in safari we must wait until the CSS are loaded and applied
...
(function() {
	...
	sap.ui.core.RenderManager.preserveContent = function(oRootNode, bPreserveRoot, bPreserveNodesWithId) {
		...
		// HACK: ignore re-rendering of static UIArea....
		if ( oRootNode.id === "sap-ui-static" ) {
			return;
		}
		...
		function check($candidates) {
			$candidates.each(function(i,candidate) {
				...
				if ( $candidate.is("[" + ATTR_PRESERVE_MARKER + "]") )  {
					$candidate.appendTo($preserve);
				} else if ( bPreserveNodesWithId && candidate.id && !(candidate.id == ID_PRESERVE_AREA)){
 ...
 * @deprecated Since version 0.15.0. Use <code>writeEscaped</code>, <code>writeAttributeEscaped</code> or directly <code>jQuery.sap.escapeHTML</code> instead. Will be removed in one of the next versions.
 * @public
 */
sap.ui.core.RenderManager.prototype.getEscaped = function(/** string */ sText) {
	if (sText) {
 ...
 *
 * @param {string} sName name of the class to be added
 * @return {sap.ui.core.RenderManager} this render manager instance to allow chaining
 * @public
 */
 ...
 * @public
 */
sap.ui.core.RenderManager.prototype.writeClasses = function() {
	var oStyle = this.aStyleStack[this.aStyleStack.length-1];
	if (oStyle.aClasses || oStyle.aCustomStyleClasses) {
		this.write(" class=\"");
		if (oStyle.aClasses) {
			this.write(oStyle.aClasses.join(" "));
		}
		if (oStyle.aCustomStyleClasses) {
			this.write(" " + oStyle.aCustomStyleClasses.join(" "));
		}
		this.write("\" ");
	}
...
/**
 * Writes the custom style classes and flushes the class collection
 * @return {sap.ui.core.RenderManager} this render manager instance to allow chaining
 * @public
 * @deprecated Since version 0.15.0. Use <code>writeClasses</code> instead. Will be removed in one of the next versions.
 */
sap.ui.core.RenderManager.prototype.writeCustomStyleClasses = function() {
	var oStyle = this.aStyleStack[this.aStyleStack.length-1];
	if (oStyle.aCustomStyleClasses) {
		this.write(" class=\"" + oStyle.aCustomStyleClasses.join(" ") + "\" ");
	}
	oStyle.aClasses = null; // TODO Needed? or should this be this.aCustomStyleClasses = null
	return this;
};
 ...
 * @public
 */
sap.ui.core.RenderManager.prototype.writeAttribute = function(sName, sValue) {
	this.write(" ", sName, "=\"", sValue, "\"");
	return this;
};
	...
	if(oControl != null) {
		...
		for(var p in mMetaProps){
			...
			case "checked": // E.g. CheckBox
				...
				break;
//			case "ARIA_and_Phoenix_do_not_perfectly_fit_to_eachother":
...
// A bridge between the jQuery.sap plugin and the Phoenix Core
...
//	jQuery.sap.ui = {};
	...
	/**
	 * Function identifying the closest Phoenix Control in the given jQuery context (provided via jQuery.map method).
	 * @private
	 */
	...
	/**
	 * Extension function to the jQuery.fn which identifies Phoenix controls in the given jQuery context.
	 *
...
sap.ui.base.Object.defineClass("sap.ui.core.UIArea", {
		// ---- object ----
		baseType : "sap.ui.base.EventProvider",
		publicMethods : ["getId", "setRootNode", "getRootNode", "setRootControl", "getRootControl", "getContent", "insertContent", "addContent", "removeContent", "removeAllContent", "indexOfContent", "destroyContent", "lock", "unlock", "isLocked", "getSavedContent", "setModel", "getModel"]
	});
...
sap.ui.core.UIArea.prototype.setRootNode = function(oRootNode) {
	...
	//TODO why not assert that oRootNode is a DOM ref.?
 ...
 * @public
 */
sap.ui.core.UIArea.prototype.addContent = function(oContent, _bSuppressRerendering) {
	this.addAggregation("content", oContent, _bSuppressRerendering);
	// TODO this remains here just to make the UX3 Shell work which doesn't invalidate properly
	if ( _bSuppressRerendering !== true ) {
		this.invalidate();
	}
	return this;
};
 ...
 * @public
 */
sap.ui.core.UIArea.prototype.removeContent = function(vContent, _bSuppressRerendering) {
	var oContent = this.removeAggregation("content", vContent, _bSuppressRerendering);
	if ( !_bSuppressRerendering ) {
		var oDomRef;
 ...
 * @public
 */
sap.ui.core.UIArea.prototype.setModel = function(oModel) {
	this.oModel = oModel;
};
 ...
 * @public
 */
sap.ui.core.UIArea.prototype.getModel = function() {
	if (this.oModel) {
		return this.oModel;
	}
	return this.oCore.getModel();
};
...
sap.ui.core.UIArea.prototype.rerender = function(force){
	...
	if (this.oRenderControl == this) {
		if (this.oRootNode) {
			...
			for(var i=0; i<len; i++){
				jQuery(this.aContentToRemove[i]).remove();
			}
			this.aContentToRemove = [];
		...
		//find out what is the topmost control/controls to render
		//get the id path of the control
		// Note: do not use "." as a separator for IDs!
		var aControlPaths = [];
		var mInvalidatedParents = {};
		for (var n in this.mInvalidatedControls) {
			var oControl = this.mInvalidatedControls[n],
				sControlPath = oControl.getId();
			oControl = oControl.getParent();
			while (oControl && oControl != this) {
				var sId = oControl.getId();
				sControlPath = sId + " " + sControlPath;
				oControl = oControl.getParent();
				if (mInvalidatedParents[sId] && oControl instanceof sap.ui.core.Control) {
					mInvalidatedParents[sId]++;
					if (mInvalidatedParents[sId] == 5) {
						this.mInvalidatedControls[sId] = oControl;
					}
				} else {
					mInvalidatedParents[sId] = 1;
				}
			}
			aControlPaths.push(sControlPath);
		}
		aControlPaths.sort();
		var sPath = "";
		for (var i=0;i<aControlPaths.length;i++) {
			if (i==0) {
				sPath = aControlPaths[0];
			} else {
				if (sPath === aControlPaths[i] || aControlPaths[i].indexOf(sPath + " ")===0 ) {
					// sPath is the same as or a true ancestor of aControlPaths[i]
					// therefore aControlPaths[i] needs not to be rendered
					aControlPaths[i] = null;
				} else {
					sPath = aControlPaths[i];
				}
			}
		}
		var aRenderControls = [];
		for (var i=0;i<aControlPaths.length;i++) {
			var sPath = aControlPaths[i];
			if (sPath) {
				var oControl = this.oCore.getElementById(sPath.substring(sPath.lastIndexOf(" ")+1));
				if (oControl) { // CSN 0000834961 2011: control may have been destroyed since invalidation happened
					oControl.rerender();
				}
			}
		}
...
sap.ui.core.UIArea.prototype._handleEvent = function(/**event*/oEvent) {
	...
	// TODO: this should be the 'lowest' Phoenix Control of this very
...
//	}
	...
	jQuery(oDomRef).bind(jQuery.sap.ControlEvents.join(" "), jQuery.proxy(this._handleEvent, this));
...
sap.ui.core.UIArea.prototype._ondetach = function() {
	...
	// unregister the all event handlers of the control
	jQuery(oDomRef).unbind();
...
sap.ui.core.Core = function() {
	...
	var oSelf = this;
	sap.ui.core.Element.prototype.register = function() {
		oSelf.registerElement(this);
	};
	sap.ui.core.Element.prototype.deregister = function() {
		oSelf.deregisterElement(this);
	};

	log.info("Declared modules: " + this.oConfiguration.modules, METHOD);
	...
	/**
	 * Retrieve the {@link sap.ui.core.Core Phoenix Core} instance for the current window.
	 * @returns the API of the current Phoenix Core instance.
	 * @public
	 * @function
	 */
	sap.ui.getCore = jQuery.sap.getter(this.getInterface());

	if (!window.document.body || window.document.readyState) {
		//the instance will boot now
		this.boot();
	} else {
		// TODO what happens if boot is not executed here? it is not executed anywhere else!
	}
	...
	jQuery(document).ready(jQuery.proxy(this, "handleLoad")); // make sure Phoenix is first when jQuery.ready is fired

};

sap.ui.core.Core.prototype = jQuery.sap.newObject(sap.ui.base.EventProvider.prototype);
sap.ui.core.Core.CoreLibName = "sap.ui.core";
sap.ui.base.Object.defineClass("sap.ui.core.Core", {
		// ---- object ----
		baseType : "sap.ui.base.EventProvider",
		publicMethods : ["isInitialized","attachInitEvent","getRenderManager","createRenderManager",
						 "getConfiguration", "setRoot", "createUIArea", "getUIArea", "getElementById",
						 ...
						 "getLoadedLibraries", "attachControlEvent", "detachControlEvent", "initLibrary",
						 "includeLibraryTheme", "setModel", "getModel"]
	});
/**
 * Map of event names and ids, that are provided by this class
 * @private
...
sap.ui.core.Core.prototype.boot = function() {
		...
		if ( b.mozilla ) {
			...
			if (document.documentMode) {
				if(document.documentMode === 7) {
					...
					jQuery.sap.log.warning("Browser running in IE7 document mode.", "", "sap.ui.core.Core");
	...
	if ( id ) {
		...
		jQuery.sap.log.debug("Browser-Id: " + id, "", "sap.ui.core.Core");
	}

	// set CSS class for the theme name
	var sTheme = this.oConfiguration.getTheme();
	jQuery(document.documentElement).addClass("sapUiTheme-" + sTheme);
...
/**
 * Executes a 'preload' for the given library.
 *
 * Creates lazy imports for all controls and elements in the library (as listed in the library.properties)
 * and includes the <code>library.js</code> script into the current document. The library.js currently
 * contains all enum types of the library.
 *
 * As a result, consuming applications don't have to write import statements for the controls or for the enums.
 *
 * @param {string} sLib name of the library to import
 * @private
 */
sap.ui.core.Core.prototype.loadLibrary = function(sLib) {
	jQuery.sap.require(sLib + ".library");
	if ( !this.mLibraries[sLib] ) {
		jQuery.sap.log.warning("library " + sLib + " didn't initialize itself");
		this.initLibrary(sLib); // TODO redundant to generated initLibrary call....
	}
};
...
sap.ui.core.Core.prototype.initLibrary = function(vLibInfo) {
	...
	log.debug("Analyzing Library " + sLibName + " -  sap.ui.core.Core.initLibrary()");
	...
	function readLibInfoFromProperties() {
		...
		log.debug("Required Libraries: " + sDepInfo + " -  sap.ui.core.Core.initLibrary()");
	...
	for(var i=0; i<oLibInfo.dependencies.length; i++) {
		...
		log.debug("resolve Dependencies to " + sDepLib + " -  sap.ui.core.Core.initLibrary()");
		if ( !this.mLibraries[sDepLib] ) {
			log.warning("Dependency from " + sLibName + " to " + sDepLib + " has not been resolved by library itself");
	...
	if ( !jQuery.sap.isDeclared(sLibName + ".library") ) {
		...
		log.warning("Library Module " + sLibName + ".library" + " not loaded automatically");
...
/**
 * returns a map which contains the names of the loaded libraries and the
 * loaded controls of the library
 * @return {map} map of library names / controls
 * @public
 */
sap.ui.core.Core.prototype.getLoadedLibraries = function() {
	// TODO secure with a deep copy
...
sap.ui.core.Core.prototype.rerenderAllUIAreas = function(){
	if ( !this._sRerenderTimer ) {
		...
		this._sRerenderTimer = jQuery.sap.delayedCall(0,this,"_rerenderAllUIAreas"); // decoupled for some reason TODO: document reason
	}
};
...
sap.ui.core.Core.prototype.fireThemeChanged = function(mParameters) {
	...
	jQuery.each(this.mElements, function(sId, oElement) {
		if (oElement.onThemeChanged) {
			oElement.onThemeChanged.apply(oElement, [oEvent]);
		}
	});
	this.oEventPool.returnObject(oEvent);
	// notify the listeners
 ...
 * @public
 */
sap.ui.core.Core.prototype.setModel = function(oModel) {
	this.oModel = oModel;
};
 ...
 * @public
 */
sap.ui.core.Core.prototype.getModel = function() {
	return this.oModel;
};
