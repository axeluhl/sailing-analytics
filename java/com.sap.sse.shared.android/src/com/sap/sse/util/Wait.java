...
public class Wait {
     ...
     * timeout, if provided in {@code optionalTimeoutEmptyMeansForever}, has expired. Sleeps
     * {@code sleepBetweenAttempts} between subsequent attempts. Note that once decided for starting another evaluation,
     * that evaluation may surpass the timeout; but no new evaluation will be started after the timeout has expired. The
     * {@code condition} will be evaluated at least once, even if a timeout of zero seconds has been specified. If no
     * timeout is provided, the method will keep evaluating the {@code callable} until it passes the
     ...
     * and that another attempt will be made in {@cod esleepBetweenAttempts} until the timeout has been reached.
     * <p>
     * 
