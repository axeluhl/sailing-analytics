...
public class WindTrackFromManeuverGraphExtractor {
    ...
    public List<WindWithConfidence<ManeuverForEstimation>> getWindTrack(
            List<Pair<GraphLevel, FineGrainedPointOfSail>> bestPath, double bestPathConfidence) {

        List<WindWithConfidence<ManeuverForEstimation>> windTrack = getWindTrackWithLastNodeOfLastLevelConsideringMiddleCoursesOfCleanTacksAndJibes(
    ...
    protected List<WindWithConfidence<ManeuverForEstimation>> getWindTrackWithLastNodeOfLastLevelConsideringMiddleCoursesOfCleanTacksAndJibes(
            List<Pair<GraphLevel, FineGrainedPointOfSail>> bestPath, double baseConfidence) {
        List<WindWithConfidence<ManeuverForEstimation>> windTrack = new ArrayList<>();
        for (ListIterator<Pair<GraphLevel, FineGrainedPointOfSail>> iterator = bestPath.listIterator(); iterator
            ...
            if (pointOfSailBeforeManeuver.getCoarseGrainedPointOfSail().getLegType() == currentNode
                ...
                if (tackOrJibe != null) {
                    WindWithConfidenceImpl<ManeuverForEstimation> windWithConfidence = getWindWithConfidenceIfTackOrJibe(
    ...
    protected List<WindWithConfidence<ManeuverForEstimation>> getWindTrackWithLastNodeOfLastLevelConsideringStableCourses(
            List<Pair<GraphLevel, FineGrainedPointOfSail>> bestPath, double baseConfidence) {
        List<WindWithConfidence<ManeuverForEstimation>> windTrack = new ArrayList<>();
        for (Pair<GraphLevel, FineGrainedPointOfSail> pathEntry : bestPath) {
            ...
            if (pointOfSailBeforeManeuver.getCoarseGrainedPointOfSail() != currentNode.getCoarseGrainedPointOfSail()) {
                ...
                if (pointOfSailForWindSpeedDetermination != null) {
                    WindWithConfidenceImpl<ManeuverForEstimation> windWithConfidence = getWindWithConfidence(
    ...
    protected List<WindWithConfidence<ManeuverForEstimation>> getWindTrackWithLastNodeOfLastLevelConsideringPointOfSailsWithoutWindSpeed(
            List<Pair<GraphLevel, FineGrainedPointOfSail>> bestPath, double baseConfidence) {
        List<WindWithConfidence<ManeuverForEstimation>> windTrack = new ArrayList<>();
        for (Pair<GraphLevel, FineGrainedPointOfSail> pathEntry : bestPath) {
            final GraphLevel currentLevel = pathEntry.getA();
            ...
            final FineGrainedPointOfSail pointOfSailBeforeManeuver = currentLevel
                    .getPointOfSailBeforeManeuver(currentNode);
            if (pointOfSailBeforeManeuver.getCoarseGrainedPointOfSail() != currentNode.getCoarseGrainedPointOfSail()) {
                WindWithConfidenceImpl<ManeuverForEstimation> windWithConfidence = getWindWithConfidenceWithoutSpeed(
    ...
    private WindWithConfidenceImpl<ManeuverForEstimation> getWindWithConfidenceWithoutSpeed(
            FineGrainedPointOfSail pointOfSail, GraphLevel currentLevel, double baseConfidence) {
        double windCourseInDegrees = currentLevel.getWindCourseInDegrees(pointOfSail);
        WindWithConfidenceImpl<ManeuverForEstimation> windWithConfidence = constructWindWithConfidence(
                windCourseInDegrees, 0, currentLevel, baseConfidence);
        return windWithConfidence;
    }

    private WindWithConfidenceImpl<ManeuverForEstimation> getWindWithConfidence(
        ...
        WindWithConfidenceImpl<ManeuverForEstimation> windWithConfidence = null;
    ...
    private WindWithConfidenceImpl<ManeuverForEstimation> getWindWithConfidenceIfTackOrJibe(GraphLevel currentLevel,
    ...
    private WindWithConfidenceImpl<ManeuverForEstimation> constructWindWithConfidence(
            SpeedWithBearing windSpeedWithCourse, GraphLevel currentLevel, double confidence) {
        ...
        WindWithConfidenceImpl<ManeuverForEstimation> windWithConfidence = new WindWithConfidenceImpl<>(wind,
                confidence, currentLevel.getManeuver(), windSpeedWithCourse.getKnots() > 0);
        return windWithConfidence;
    }

    private WindWithConfidenceImpl<ManeuverForEstimation> constructWindWithConfidence(double windCourseDeg,
            double windSpeedKnots, GraphLevel currentLevel, double confidence) {
