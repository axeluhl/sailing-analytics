...
public class ManeuverEstimationResult {
    ...
    private void computeLikelihoodsForPointOfSailAfterManeuver() {
        ...
        for (CoarseGrainedPointOfSail pointOfSailAfterManeuver : CoarseGrainedPointOfSail.values()) {
            ...
            case _360:
                likelihood = getLikelihoodForPointOfSailAfterPenaltyCircle(courseChangeDeg,
                        courseChangeDegUntilLowestSpeed, pointOfSailAfterManeuver);
                break;
            case REGULAR:
            case MARK_PASSING:
                likelihood = getLikelihoodForPointOfSailAfterRegularManeuver(maxLikelihood, courseChangeDeg,
                        pointOfSailAfterManeuver);
                break;
            case _180:
            case SMALL:
            case WIDE:
    ...
    private double getLikelihoodForPointOfSailAfterPenaltyCircle(double courseChangeDeg,
            double courseChangeDegUntilLowestSpeed, CoarseGrainedPointOfSail pointOfSailAfterManeuver) {
        ...
        if (courseChangeDegUntilLowestSpeed < 0) {
            if (courseChangeDegUntilLowestSpeed >= -100) {
                if (possiblePointOfSailsBeforeManeuver.contains(CoarseGrainedPointOfSail.UPWIND_STARBOARD)) {
                    addLikelihood = true;
                }
            } else {
                ...
                if (courseChangeDegUntilLowestSpeed >= -100 || courseChangeDegUntilLowestSpeed <= -225) {
                    if (possiblePointOfSailsBeforeManeuver.stream()
                            .anyMatch(pointOfSailBefore -> pointOfSailBefore.getLegType() == LegType.UPWIND)) {
                        addLikelihood = true;
                    }
                } else {
                    if (possiblePointOfSailsBeforeManeuver.stream()
                            .anyMatch(pointOfSailBefore -> pointOfSailBefore.getLegType() == LegType.DOWNWIND)) {
                        addLikelihood = true;
                    }
                }
            }
        ...
        double likelihood = addLikelihood ? 0.7 : 0.3;
        return likelihood;
    }

    private double getLikelihoodForPointOfSailAfterRegularManeuver(double maxLikelihood, double courseChangeDeg,
            CoarseGrainedPointOfSail pointOfSailAfterManeuver) {
        double likelihood = 0;
        for (CoarseGrainedManeuverType maneuverType : CoarseGrainedManeuverType.values()) {
            if (likelihoodPerManeuverType[maneuverType.ordinal()] != 0) {
                ...
                case HEAD_UP:
                    if (Math.abs(courseChangeDeg) > 40) {
                        if (courseChangeDeg < 0 && pointOfSailAfterManeuver == CoarseGrainedPointOfSail.UPWIND_STARBOARD
                                || (courseChangeDeg >= -80
                                        && pointOfSailAfterManeuver == CoarseGrainedPointOfSail.REACHING_STARBOARD)
                                || courseChangeDeg > 0
                                        && pointOfSailAfterManeuver == CoarseGrainedPointOfSail.UPWIND_PORT
