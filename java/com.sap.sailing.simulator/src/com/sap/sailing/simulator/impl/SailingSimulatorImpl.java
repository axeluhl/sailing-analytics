...
public class SailingSimulatorImpl implements SailingSimulator {
    ...
    private durP calcDuration(TimePoint curtime, int side1, Position p1, Position p2, WindField windField, PolarDiagram polarDiagram) {
    ...
    // and path index pidx (pidx currently unused, maybe used for structuring in groups of equivalent time, see prototype R-script) 
    class durP {
    ...
    private Path createDP() {
        ...
        int sgridv = (int) Math.round(1.5*simulationParameters.getProperty("Djikstra.gridv[int]").intValue()); // number of vertical grid steps
        int sgridh = (int) Math.round(1.5*simulationParameters.getProperty("Djikstra.gridh[int]").intValue()); // number of horizontal grid steps
        ...
        int maxeqpaths = 2;  // maximum nmber of equal paths to keep [currently set to low values, to save Java heap memory]
        ...
        for(int idxv=0; idxv<(gridv-1); idxv++) {
            ...
            for(int idxh2=-range2; idxh2<=range2; idxh2++) { 
                ...
                for(int idxh1=-range1; idxh1<=range1; idxh1++) { 
                    ...
                    durP durationh1h2 = calcDuration(new MillisecondsTimePoint(duras.get(idxh1+range1).duration),duras.get(idxh1+range1).side, sailGrid[idxv][idxh1+gridh], sailGrid[idxv+1][idxh2+gridh], windField, polarDiagram);
                ...
                if (idxv>0) { // if not at start of the grid
                    ...
                    tpaths.add(tpath);
                    pathsnew.add(tpaths);
                    durP tdur = calcDuration(startTime,0,sailGrid[0][gridh],sailGrid[1][idxh2+gridh],windField,polarDiagram);
            ...
            /*if (idxv == (gridv-2)) {
                ...
                for(int i=0; i<1;i++) {//paths.size(); i++) {
                    //System.out.print(""+duras.get(i).duration+" - ");
                    for(int j=0; j<1;j++) {//paths.get(i).size(); j++) {
                        int kmax = paths.get(i).get(j).size();
                        System.out.print(""+((paths.get(i).get(j).get(kmax-1).dur-paths.get(i).get(j).get(0).dur)/1000./60.)+"min: ");
                        for(int k=0;k<kmax; k++) {
                            System.out.print(" "+paths.get(i).get(j).get(k).idx);
                        }
                        System.out.println();
                    }  
                }
