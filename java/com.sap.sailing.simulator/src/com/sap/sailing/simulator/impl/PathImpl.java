...
public class PathImpl implements Path, Serializable {
	...
	public List<TimedPositionWithSpeed> getEvenTimedPath(final long timeStep) {
		...
		while (idx < this.pathPoints.size()) {
			...
			if (this.pathPoints.get(idx).getTimePoint().asMillis() >= nextTimePoint.asMillis()) {
				...
				final TimedPositionWithSpeed nextPoint = new TimedPositionWithSpeedImpl(nextTimePoint, nextPosition, nextWind);
				...
				final double scaleDist = 0.01 * nextPoint.getPosition().getDistance(prevPoint.getPosition()).getMeters();
	...
	public TimedPositionWithSpeed getPositionAtTime(final TimePoint t) {
		...
		final Distance dist = p1.getPosition().getDistance(p2.getPosition());
		final Position p0 = p1.getPosition().translateGreatCircle(p1.getPosition().getBearingGreatCircle(p2.getPosition()),
	...
	private static boolean isPosition2Turn(final Position pos1, final Position pos2, final Position pos3, final double treshold)
	{
