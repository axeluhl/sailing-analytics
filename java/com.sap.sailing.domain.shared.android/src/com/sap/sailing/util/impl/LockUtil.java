...
public class LockUtil {
        ...
        while (!locked) {
            synchronized (getCurrentThreadsLockCounts()) {
                if (!ifIsInCurrentThreadsLockSetThenIncrementLockCount(lock.readLock())) {
                    ...
                    logger.finest("only incremented count for read lock " + lock.getName() + " in thread "
    ...
    public static void unlockAfterRead(NamedReentrantReadWriteLock lock) {
        ...
        synchronized (currentThreadLockCounts) {
            ...
            if (countForLock == 1 || lock.getReaders().contains(Thread.currentThread())) {
                ...
                decrementLockCountForCurrentThread(lock.readLock());
                logger.finest("only decrementing counter for read lock "+lock.getName()+" in thread "+Thread.currentThread().getName()+" from "+countForLock);
            }
        }
    }
        ...
        while (!locked) {
            synchronized (getCurrentThreadsLockCounts()) {
                if (!ifIsInCurrentThreadsLockSetThenIncrementLockCount(lock.writeLock())) {
                    locked = lock(lock.writeLock(), lock.getWriteLockName(), lock);
                    if (locked) {
                        logger.finest("actually acquired write lock " + lock.getName() + " in thread " + Thread.currentThread().getName());
                    ...
                    logger.finest("only incremented count for write lock " + lock.getName() + " in thread "
                            + Thread.currentThread().getName());
                    locked = true;
                }
            }
    ...
    public static void unlockAfterWrite(NamedReentrantReadWriteLock lock) {
        ...
        Map<Lock, Integer> currentThreadLockCounts = getCurrentThreadsLockCounts();
        synchronized (currentThreadLockCounts) {
            final Integer countForLock = currentThreadLockCounts.get(lock.writeLock());
            if (countForLock == 1) {
                logger.finest("actually unlocking write lock "+lock.getName()+" in thread "+Thread.currentThread().getName());
                ...
                if (timePointWriteLockWasObtained == null) {
                    ...
                    if (now.asMillis() - timePointWriteLockWasObtained.asMillis() > 10000l) {
                        ...
                        logger.info("write lock " + lock.getName()
                                + " was held for more than 10s. It got unlocked here: " + stackTrace);
                    }
                }
            } else {
                decrementLockCountForCurrentThread(lock.writeLock());
                logger.finest("only decrementing counter for write lock "+lock.getName()+" in thread "+Thread.currentThread().getName()+" from "+countForLock);
            }
        }
    }
