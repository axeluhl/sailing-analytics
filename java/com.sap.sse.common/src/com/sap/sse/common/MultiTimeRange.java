...
/**
 * A minimal sequence of non-overlapping, non-touching {@link TimeRange} objects. The iteration order is from earlier to
 * later time range. An object of this type may be empty. Only the last {@link TimeRange} in the iteration may be
 * unbounded at its end, and only the first {@link TimeRange} in the iteration may be unbounded at its start. Being
 * minimal means that no other {@link MultiTimeRange} can represent the exact same set of {@link TimePoint}s with fewer
 * {@link TimeRange} elements in it.<p>
 * 
