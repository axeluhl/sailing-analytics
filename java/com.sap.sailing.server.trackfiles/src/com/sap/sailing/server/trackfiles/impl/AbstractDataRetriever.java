...
public abstract class AbstractDataRetriever implements DataRetriever {
	...
	public abstract Collection<GpxRoute> getRoutes(TrackFilesFormat format,
	...
	@Override
	public byte[] getData(TrackFilesFormat format, TrackedRace race,
			boolean dataBeforeAfter, boolean rawFixes)
			throws FormatNotSupportedException, IOException {
		Collection<GpxRoute> routes = getRoutes(format, race, dataBeforeAfter,
				rawFixes);
	...
	private <F extends BaseNavigationFormat<? extends BaseRoute>> byte[] getRoutesAsBytes(
			List<BaseRoute> routes, F format) throws IOException,
		...
		if (format instanceof MultipleRoutesFormat) {
			...
			throw new FormatNotSupportedException(
					"Format cannot contain multiple routes per file");
		}
		return out.toByteArray();
	}

	protected <E, F extends Timed, T> Collection<GpxRoute> getRoutes(
			TrackedRace race, boolean dataBeforeAfter, boolean rawFixes,
			WaypointCreator<F> creator, Iterable<E> elements,
			NameReader<E> nameReader, TrackReaderRetriever<E, F> trackRetriever)
			throws FormatNotSupportedException, IOException {
		List<GpxRoute> routes = new ArrayList<>();
		for (E element : elements) {
			TimePoint start = race.getStartOfRace() != null ? race
					.getStartOfRace() : race.getStartOfTracking();
			TimePoint end = race.getEndOfRace() != null ? race.getEndOfRace()
					: race.getEndOfTracking();

			String name = nameReader.getName(element);
			GpxRoute route = new GpxRoute(new Gpx11Format(),
			...
			try {
				Iterable<F> fixesIter = rawFixes ? trackReader
						.getRawTrack(element) : trackReader.getTrack(element);
				if (fixesIter == null) {
					continue;
				}
				int i = 0;
