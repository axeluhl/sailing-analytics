...
public class MetaDataRetriever implements DataRetriever {
	...
	@Override
	public byte[] getData(TrackFilesFormat format, final TrackedRace race,
		...
		if (race.getStartOfRace() != null)
			pw.println("START_OF_RACE\t"
					+ f.format(race.getStartOfRace().asDate()));
		if (race.getEndOfRace() != null)
			pw.println("END_OF_RACE\t" + f.format(race.getEndOfRace().asDate()));
		if (race.getStartOfTracking() != null)
			pw.println("START_OF_TRACKING\t"
					+ f.format(race.getStartOfTracking().asDate()));
		if (race.getEndOfTracking() != null)
			pw.println("END_OF_TRACKING\t"
					+ f.format(race.getEndOfTracking().asDate()));
		pw.println();
		pw.flush();
