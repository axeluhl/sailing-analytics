...
public class LoadBalancerRuleInserter<ShardingKey, RA extends com.sap.sse.landscape.aws.impl.LoadBalancerRuleInserter.RuleAdapter<RA>> {
    ...
    public Iterable<RA> getRulesSortedByPriority() {
        return Util.stream(loadBalancerAdapter.getRules())
                .filter(t -> !t.priority().equalsIgnoreCase(ApplicationLoadBalancer.DEFAULT_RULE_PRIORITY))
                .sorted((r1, r2) -> Integer.compare(Integer.valueOf(r1.priority()),
                        Integer.valueOf(r2.priority())))::iterator;
    }
