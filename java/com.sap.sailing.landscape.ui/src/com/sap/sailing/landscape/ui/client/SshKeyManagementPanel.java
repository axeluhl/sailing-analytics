...
public class SshKeyManagementPanel extends VerticalPanel {
    ...
    public SshKeyManagementPanel(StringMessages stringMessages, UserService userService,
        ...
        addButton.setEnabled(
                regionSelectionModel.getSelectedObject() != null && awsAccessKeyProvider.hasValidSessionCredentials());
        ...
        generateButton.setEnabled(
                regionSelectionModel.getSelectedObject() != null && awsAccessKeyProvider.hasValidSessionCredentials());
        sshKeyTable = new TableWrapperWithSingleSelectionAndFilter<SSHKeyPairDTO, StringMessages, AdminConsoleTableResources>(
                stringMessages, errorReporter, /* enablePager */ true,
                ...
                /* filter label */ Optional.empty(),
                /* filter checkbox label */ stringMessages.hideElementsWithoutUpdateRights()) {
            @Override
        ...
        SecuredDTOOwnerColumn.configureOwnerColumns(sshKeyTable.getTable(), sshKeyTable.getColumnSortHandler(),
                stringMessages);
        ...
        final DialogConfig<SSHKeyPairDTO> editOwnerShipDialog = EditOwnershipDialog.create(
                userService.getUserManagementWriteService(), SecuredLandscapeTypes.SSH_KEY,
        ...
        final EditACLDialog.DialogConfig<SSHKeyPairDTO> configACL = EditACLDialog.create(
                userService.getUserManagementWriteService(), SecuredLandscapeTypes.SSH_KEY, null, stringMessages);
        ...
        sshKeyPairActionColumn.addAction(SshKeyPairImagesBarCell.ACTION_SHOW_KEYS, DefaultActions.READ,
                sshKeyPairDTO -> landscapeManagementService.getSshPublicKey(sshKeyPairDTO.getRegionId(),
                        sshKeyPairDTO.getName(), new AsyncCallback<byte[]>() {
                            @Override
                            public void onFailure(Throwable caught) {
                                errorReporter.reportError(caught.getMessage());
                            }
                            ...
                            public void onSuccess(final byte[] publicKey) {
                                landscapeManagementService.getEncryptedSshPrivateKey(sshKeyPairDTO.getRegionId(),
                                        sshKeyPairDTO.getName(), new AsyncCallback<byte[]>() {
                                            @Override
                                            public void onFailure(Throwable caught) {
                                                errorReporter.reportError(caught.getMessage());
                                            ...
                                            public void onSuccess(final byte[] encryptedPrivateKey) {
                                                showKeys(sshKeyPairDTO.getName(), publicKey, encryptedPrivateKey,
                                                        stringMessages);
                                            }
                                        });
                            }
        ...
        buttonPanel.addRemoveAction(stringMessages.remove(), sshKeyTable.getSelectionModel(),
                /* withConfirmation */ true, () -> {
                    landscapeManagementService.removeSshKey(sshKeyTable.getSelectionModel().getSelectedObject(),
    ...
    private void openGenerateSshKeyDialog(StringMessages stringMessages, AwsAccessKeyProvider awsAccessKeyProvider) {
        new GenerateSshKeyDialog(stringMessages, new DialogCallback<Triple<String, String, String>>() {
            @Override
    ...
    private void openAddSshKeyDialog(StringMessages stringMessages, AwsAccessKeyProvider awsAccessKeyProvider) {
        new AddSshKeyDialog(stringMessages, new DialogCallback<Triple<String, String, String>>() {
            @Override
            public void ok(Triple<String, String, String> keyPairNameAndPublicAndPrivateKey) {
                landscapeManagementService.addSshKeyPair(regionSelectionModel.getSelectedObject(),
                        keyPairNameAndPublicAndPrivateKey.getA(), keyPairNameAndPublicAndPrivateKey.getB(),
                        keyPairNameAndPublicAndPrivateKey.getC(), new AsyncCallback<SSHKeyPairDTO>() {
                            @Override
                            public void onFailure(Throwable caught) {
                                errorReporter.reportError(caught.getMessage());
                            }
